...
Dim ch$
Dim icon$
Dim is_complete_display As Boolean
...
ch$ = Mid$(A$, i%, 1)
            If ch$ = Chr(13) Then
             is_complete_display = True
            ElseIf ch$ = Chr(12) Then  '"," Then
...
'Next i%
If find_conclusion1(0, 0, True) > 0 Then '检查结论是否已在输入的条件中
 GoTo method_mark1
'Else
' Call pre_set_reduce
...
ElseIf error_of_wenti = 1 Then '问题有错
...
'Call set_dispaly
 ...
 '*****************************************************************
For i% = 0 To last_conclusion - 1
Call set_display_string__(conclusion_data(i%).ty, conclusion_data(i%).no(0))
Next i%
    Call C_display_wenti.set_m_no(profe_, 1, wenti_no%)
    Call C_display_wenti.set_m_string(wenti_no%, "", LoadResString_(1700, ""), "", "", "", _
                   0, condition, 1)
'*****************************************************************************
...
Public Function set_display_inform(ByVal ts$, ty As Integer) As String
Dim i%
Dim ch As String
For i% = 1 To Len(ts$)
 ch = Mid$(ts$, i%, 1)
...
Else
 i% = InStr(1, ts, "[", 0)
 j% = InStr(1, ts, "_", 0) '
 If (i% > 0 And i% < j%) Or j% = 0 Then '[_ or {
  read_string_from_string_for_print = read_string_from_string(1, ts, "[", "]", 0, s1, S2) 's1+读出[]中的内容+s2
 ElseIf j% > 0 Then '_
  read_string_from_string_for_print = read_string_from_string(1, ts, "_", "~", 0, s1, S2) 's1+读出_~中的内容+s2
  S2 = "~" + S2
  'S2 = "_"
 End If
 '读出"[]"
If read_string_from_string_for_print <> "" Then '读出的内容非空
   s3 = read_string_from_string(1, read_string_from_string_for_print, "\\", "\\", 0, "", "")
 If s3 <> "" Then
    Call replace_string_by_string(read_string_from_string_for_print, s3, global_icon_char)
