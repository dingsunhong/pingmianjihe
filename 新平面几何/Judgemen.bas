...
Function is_four_point_on_circle(ByVal p1%, ByVal p2%, ByVal p3%, ByVal p4%, _
       no%, p4_c As four_point_on_circle_data_type, set_or_judg As Boolean) As Boolean
'判断是否共圆，并排序
 '不共圆输出-1
Dim i%, j%, p%, c%
Dim tp(3) As Integer
 no% = 0
If p1% = p2% Or p1% = p3% Or p1% = p4% Or p2% = p3% Or p2% = p4% Or p3% = p4% Or _
      p1% = 0 Or p2% = 0 Or p3% = 0 Or p4% = 0 Then
...
For i% = 2 To 0 Step -1
 ...
 Next j%
Next i%
'If arrange_four_point_on_circle(tp(0), tp(1), tp(2), tp(3)) Then '排序
'   op1% = tp(0)
'   op2% = tp(1)
'   op3% = tp(2)
'   op4% = tp(3)
...
If is_four_point_in_epolygon(tp(0), tp(1), tp(2), tp(3), 0, 0, "") Then
 ...
 '***************************************************************************
is_four_point_on_circle = search_for_four_point_on_circle(p4_c, no%)
End Function
...
is_equal_angle = is_three_angle_value(A1%, A2%, 0, "1", "-1", "0", _
  "0", "0", n1%, n2%, 0, -1000, 0, 0, 0, 0, 0, 0, 0, a3_v, re, 0)
  n1% = re.condition(1).no
  n2% = re.condition(2).no
'*********************************************************
End Function
...
'********************************************************************************************
'有一个角已知，做替换,并将增加记录
Call reduce_used_angle0(A1%, para1, v, re)
Call reduce_used_angle0(A2%, para2, v, re)
Call reduce_used_angle0(A3%, para3, v, re)

'If A1% > 0 And angle(A1%).data(0).value <> "" Then
'   v = minus_string(v, time_string(angle(A1%).data(0).value, para1, True, False), True, False)
'    Call add_condition_to_record(angle3_value_, angle(A1%).data(0).value_no, re, 0)
'     A1% = 0
'      para1 = "0"
'End If
'If A2% > 0 And angle(A2%).data(0).value <> "" Then
'   v = minus_string(v, time_string(angle(A2%).data(0).value, para1, True, False), True, False)
'    Call add_condition_to_record(angle3_value_, angle(A2%).data(0).value_no, re, 0)
'     A2% = 0
'      para2 = "0"
'End If
'If A3% > 0 And angle(A3%).data(0).value <> "" Then
'   v = minus_string(v, time_string(angle(A3%).data(0).value, para1, True, False), True, False)
'    Call add_condition_to_record(angle3_value_, angle(A3%).data(0).value_no, re, 0)
'     A3% = 0
'      para3 = "0"
'End If
'*********************************************************************************************
...
'If A3_v.angle(2) > 0 Then
  Call combine_two_angle_with_para(a3_v.angle(1), a3_v.angle(2), _
           a3_v.para(1), a3_v.para(2), a3_v.value, 1)
'End If
'If A3_v.angle(1) > 0 Then
Call combine_two_angle_with_para(a3_v.angle(0), a3_v.angle(1), _
          a3_v.para(0), a3_v.para(1), a3_v.value, 1)
'End If
'If A3_v.angle(2) > 0 Then
  Call combine_two_angle_with_para(a3_v.angle(0), a3_v.angle(2), _
           a3_v.para(0), a3_v.para(2), a3_v.value, 1)
'End If
'******************************************************************************************************
If a3_v.angle(0) = 0 And a3_v.angle(1) = 0 And a3_v.angle(2) = 0 Then
  If a3_v.value = "0" Then
     no1% = 0
      is_three_angle_value = True
       Exit Function
  ElseIf InStr(1, a3_v.value, "y", 0) > 0 Then '含未知数
    v = solve_first_order_equation(a3_v.value, "0", "y") '解出未知数
...
If (a3_v.angle(0) = 0 Or a3_v.angle(0) > a3_v.angle(1)) And a3_v.angle(1) > 0 Then
 ...
 Call exchange_string(a3_v.para(0), a3_v.para(1))
End If
If a3_v.para(0) <> "0" And a3_v.para(0) <> "" Then '首项系数标准化工
Call simple_multi_string0(a3_v.para(0), a3_v.para(1), a3_v.para(2), a3_v.value, ts, True)
a3_v.value_ = a3_v.value
Else
ts = "1"
'*****************************************************************************************"
End If
...
If search_for_three_angle_value(a3_v, 0, no1%, 0) Then '5.7
 Call add_condition_to_record(angle3_value_, no1%, re, 0)
   ...
   tangent_line(i%).data(0) = tan_l
    Call add_record_to_record(re.data0.condition_data, tangent_line(i%).data(0).record.data0.condition_data)
     ...
     is_tangent_line = 1
      Exit Function
  ElseIf c1.no > 0 Then
   If c1.no = tangent_line(i%).data(0).ele(0).no Then
         ...
         Else
          n% = i%
           is_tangent_line = 2
            Exit Function
         End If
   ElseIf c1.no = tangent_line(i%).data(0).ele(1).no Then
        n% = i%
         is_tangent_line = 2
          Exit Function
   End If
  Else 'c2%>0
   If c2.no = tangent_line(i%).data(0).ele(0).no Then
         If tangent_line(i%).data(0).ele(1).no > 0 Then
          n% = i%
           is_tangent_line = 2
            Exit Function
         Else
          n% = i%
           is_tangent_line = 2
            Exit Function
         End If
   ElseIf c2.no = tangent_line(i%).data(0).ele(1).no Then
        n% = i%
         is_tangent_line = 2
          Exit Function
   End If
...
Else 'l1%=l2%
 ol1% = l1%
  ol2% = l2%
   Call is_three_point_on_line(p1%, p2%, p3%, p3_con_l(0), -1000, 0, 0, 0, 0, 0)
   Call is_three_point_on_line(p1%, p2%, p4%, p3_con_l(1), -1000, 0, 0, 0, 0, 0)
   Call is_three_point_on_line(p1%, p4%, p3%, p3_con_l(2), -1000, 0, 0, 0, 0, 0)
   Call is_three_point_on_line(p2%, p4%, p3%, p3_con_l(3), -1000, 0, 0, 0, 0, 0)
...
If c% = 0 Then
record_0.data0.condition_data.condition_no = 0 'record0
 If is_three_point_on_line(p1%, p2%, p3%, n%, -1000, 0, 0, _
         0, 0, 0) Then
  If c% = 0 Then
...
' If m_poi(p2%).data(0).no_reduce = 0 Then
  If is_three_point_on_line(p1%, p2%, p3%, 0, -1000, 0, 0, _
        0, 0, 0) Then
         is_diameter = True
  End If
' Else
...
End If
End Function
Public Function combine_two_angle_with_para(A1%, A2%, _
                 para1 As String, para2 As String, v$, _
                   ty_ As Byte) As Boolean 'ty_=0 , 合并ty_=1 推理
'1 是否对顶角
'3 同顶点角合并
'4 共边角合并
Dim ty As Integer
Dim t_a_no%
Dim temp_A(2) As Integer
Dim temp_v As String
Dim temp_para(1) As String
Dim t_A(2) As Integer
Dim relation_of_para As Integer
'****************************************************
temp_A(0) = A1%
temp_A(1) = A2%
temp_para(0) = para1
...
End If
'**********************************************************************************************
If temp_para(0) = temp_para(1) Then
 relation_of_para = 1
ElseIf temp_para(0) = time_string(temp_para(1), "-1", True, False) Then
 relation_of_para = -1
End If
'**********************************************************
If A1% = A2% And relation_of_para = -1 Then
  A1% = 0
  A2% = 0
  para1 = "0"
  para2 = "0"
...
End If
'*******************************************************
If angle(A1%).data(0).total_no_ Mod 2 = 0 Then '
  temp_para(0) = time_string(temp_para(0), "-1", True, False) '化为全角标准
  temp_v = add_string(temp_v, time_string(temp_para(0), "180", True, False), True, False)
End If
If angle(A2%).data(0).total_no_ Mod 2 = 0 Then
  temp_para(1) = time_string(temp_para(1), "-1", True, False)
  temp_v = add_string(temp_v, time_string(temp_para(1), "180", True, False), True, False)
End If
'************************************************************************************************
If combine_two_total_angle_for_reduce(angle(A1%).data(0).total_no, _
     angle(A2%).data(0).total_no, t_a_no%, ty) Then
  '**********************************************************************************************
           temp_A(2) = T_angle(t_a_no%).data(0).angle_no(T_angle(t_a_no%).data(0).is_used_no).no  '全角的代表角
     If ty = A1_equal_A2 Then
           temp_para(0) = add_string(temp_para(0), temp_para(1), True, False)
           temp_para(1) = "0"
            If T_angle(t_a_no%).data(0).is_used_no Mod 2 = 0 Then
             temp_para(0) = time_string(temp_para(0), "-1", True, False)
             temp_v = add_string(temp_v, time_string("180", temp_para(0), True, False), True, False)
            End If
     ElseIf ty = A1_add_A2_add_A3_equal_360 Then
        If relation_of_para = 1 Then
           temp_para(0) = time_string(temp_para(0), "-1", True, False)
           temp_para(1) = "0"
           temp_v = add_string(temp_v, time_string("360", temp_para(0), True, False), True, False)
           temp_A(0) = temp_A(2)
           temp_A(1) = 0
            If T_angle(t_a_no%).data(0).is_used_no Mod 2 = 0 Then
             temp_para(0) = time_string(temp_para(0), "-1", True, False)
             temp_v = add_string(temp_v, time_string("180", temp_para(0), True, False), True, False)
            End If
        Else 'If relation_of_para = 0 Then
           Exit Function
        End If
     ElseIf ty = A1_add_A2_equal_A3 Then
        If relation_of_para = 1 Then
           temp_para(1) = "0"
           temp_A(0) = temp_A(2)
           temp_A(1) = 0
        ...
        Else 'If relation_of_para = 0 Then
           Exit Function
        End If
     ElseIf ty = A1_minus_A2_equal_A3 Then
     If relation_of_para = -1 Then
        temp_para(1) = "0"
        temp_A(0) = temp_A(2)
        temp_A(1) = 0
            ...
            End If
       Else 'If relation_of_para = 0 Then
         temp_para(1) = minus_string(temp_para(1), temp_para(0), True, False)
           If angle(A2%).data(0).total_no_ Mod 2 = 0 Then '
             temp_para(1) = time_string(temp_para(1), "-1", True, False) '化为全角标准
             temp_v = add_string(temp_v, time_string(temp_para(1), "180", True, False), True, False)
           End If
           temp_A(0) = temp_A(2)
            If T_angle(t_a_no%).data(0).is_used_no Mod 2 = 0 Then
            ...
            End If
       End If
     ElseIf ty = A2_minus_A1_equal_A3 Then
      If relation_of_para = -1 Then
      temp_para(0) = temp_para(1)
      temp_para(1) = 0
      temp_A(0) = temp_A(2)
      temp_A(1) = 0
            If T_angle(t_a_no%).data(0).is_used_no Mod 2 = 0 Then
            ...
            End If
      Else 'If relation_of_para = 0 Then
         temp_para(0) = minus_string(temp_para(0), temp_para(1), True, False)
           If angle(A1%).data(0).total_no_ Mod 2 = 0 Then '
             temp_para(0) = time_string(temp_para(0), "-1", True, False) '化为全角标准
             temp_v = add_string(temp_v, time_string(temp_para(0), "180", True, False), True, False)
           End If
           temp_A(1) = temp_A(2)
            If T_angle(t_a_no%).data(0).is_used_no Mod 2 = 0 Then
             temp_para(0) = time_string(temp_para(1), "-1", True, False)
             temp_v = add_string(temp_v, time_string("180", temp_para(1), True, False), True, False)
            End If
      End If
     End If
'******************************************************************************
...
'   End If
'End If
'Dim angle_ty As Byte '判断是对顶角=0还是互补角,=1
      '****************************
' If angle(A1%).data(0).poi(1) = angle(A2%).data(0).poi(1) Then '共顶点
'    angle_ty = (angle(A1%).data(0).total_no_ + 4 - angle(A2%).data(0).total_no_) Mod 2
'   If para1 = para2 And (para1 <> "" Or para1 <> "0") Then
'      If angle(A1%).data(0).total_no = angle(A2%).data(0).total_no Then
'         If angle_ty = 0 Then '(angle(A1%).data(0).total_no_ + 4 - angle(A2%).data(0).total_no_) Mod 2 = 0 Then '对顶角
'            A2% = 0
'            para1 = time_string("2", para1, True, False)
'            para2 = "0"
'         Else 'If Abs(angle(A1%).data(0).total_no_ - angle(A2%).data(0).total_no_) Mod 2 = 1 Then '互补角
'            v$ = minus_string(v$, time_string("180", para1, True, False), True, False)
'            para1 = "0"
'            para2 = "0"
'            A1% = 0
'            A2% = 0
'         End If
'      ElseIf add_two_angle(A1%, A2%, para1, para2, v$) Then
'      ElseIf add_two_angle(A2%, A1%, para2, para1, v$) Then
'      End If
'**********************************************************************************
'   ElseIf para1 = time_string("-1", para2, True, False) And (para1 <> "" Or para1 <> "0") Then
'      If angle(A1%).data(0).total_no = angle(A2%).data(0).total_no Then
'         If angle_ty = 0 Then '(angle(A1%).data(0).total_no_ + 4 - angle(A2%).data(0).total_no_) Mod 2 = 0 Then '对顶角
'            A1% = 0
'            A2% = 0
'            para1 = "0"
'            para2 = "0"
'         Else 'If (angle(A1%).data(0).total_no_ + 4 - angle(A2%).data(0).total_no_) Mod 2 = 1 Then '互补角
'            v$ = add_string(v$, time_string("180", para1, True, False), True, False)
'            para1 = time_string(para1, "2", True, False)
'            para2 = "0"
'            A1% = 0
'            A2% = 0
'         End If
'      ElseIf minus_two_angle(A1%, A2%, para1, para2, v$, 0) Then
'      ElseIf minus_two_angle(A1%, A2%, para1, para2, v$, 1) Then
'      End If
' End If
...
End Function
Public Function is_condition_in_record_(con_ty As Integer, con_no As Integer, _
...
If last_conditions.last_cond(1).area_of_circle_no = last_conditions.last_cond(2).area_of_circle_no Then
ReDim Preserve area_of_circle(last_conditions.last_cond(2).area_of_circle_no + 10) As area_of_circle_type
last_conditions.last_cond(2).area_of_circle_no = last_conditions.last_cond(2).area_of_circle_no + 10
End If
...
If is_angle_value(Abs(angle_number(p1%, p2%, p3%, 0, 0)), v, "", tn(0), temp_record1.data0.condition_data) Then
 ...
 If last_conditions.last_cond(1).area_of_fan_no = last_conditions.last_cond(2).area_of_fan_no Then
 ReDim Preserve Area_of_fan(last_conditions.last_cond(2).area_of_fan_no + 10) As area_of_fan_type
 last_conditions.last_cond(2).area_of_fan_no = last_conditions.last_cond(2).area_of_fan_no + 10
 End If
...
If is_line_value(m_Circ(c%).data(0).data0.center, m_Circ(i%).data(0).data0.in_point(1), _
     0, 0, 0, "", tn%, -1000, 0, 0, 0, line_value_data0) = 1 Then
If last_conditions.last_cond(1).sides_length_of_circle_no = last_conditions.last_cond(2).sides_length_of_circle_no Then
ReDim Preserve Sides_length_of_circle(last_conditions.last_cond(2).sides_length_of_circle_no + 10) As sides_length_of_circle_type
last_conditions.last_cond(2).sides_length_of_circle_no = last_conditions.last_cond(2).sides_length_of_circle_no + 10
End If
...
Public Function is_total_angle(ByVal l1%, ByVal l2%, n%, t_A As total_angle_data_type) As Boolean
  If l1% < l2% Then
      t_A.line_no(0) = l1%
      t_A.line_no(1) = l2%
  Else
      ...
      t_A.line_no(1) = l1%
  End If
 If search_for_total_angle(t_A, n%) Then
  is_total_angle = True
 Else
  n% = 0
 End If
...
Public Sub is_conclusion_no(ByVal ty As Integer, n() As Integer)
Dim i%
For i% = 0 To last_conclusion - 1
...
Public Sub reduce_used_angle0(angle_no As Integer, t_para As String, t_value As String, re As condition_data_type)
Dim t_A%
If angle_no > 0 And angle(angle_no).data(0).is_used = False Then
  'Exit Sub
'Else
  'If angle(angle_no).data(0).is_used = False Then
      t_A% = angle(angle_no).data(0).total_no
        If (T_angle(t_A%).data(0).is_used_no + 4 - angle(angle_no).data(0).total_no_) Mod 2 = 0 Then
           angle_no = T_angle(t_A%).data(0).angle_no(T_angle(t_A%).data(0).is_used_no).no
        Else 'If Abs(T_angle(t_A%).data(0).is_used_no+4 - angle(angle_no).data(0).total_no_) Mod 2 = 1 Then
           t_para = time_string("-1", t_para, True, False)
           t_value = add_string(t_value, time_string(t_para, "180", False, False), True, False)
           angle_no = T_angle(t_A%).data(0).angle_no(T_angle(t_A%).data(0).is_used_no).no
        End If
  'End If
End If
If angle_no > 0 And angle(angle_no).data(0).value <> "" Then
   t_value = minus_string(t_value, time_string(angle(angle_no).data(0).value, t_para, True, False), True, False)
    Call add_condition_to_record(angle3_value_, angle(angle_no).data(0).value_no, re, 0)
     angle_no = 0
      t_para = "0"
End If
...
Public Function add_two_angle(A1%, A2%, _
                para1 As String, para2 As String, v$) As Boolean
Dim A%
      'If A1% = 0 Or A2% = 0 Then
      '  Exit Function
      'ElseIf angle(A%).data(0).poi(1) <> angle(A2%).data(0).poi(1) Then
      '  Exit Function
      'Else
      If angle(A1%).data(0).line_no(0) = angle(A2%).data(0).line_no(1) Then
         If angle(A1%).data(0).te(0) = angle(A2%).data(0).te(1) Then '共线同向
            A% = angle_number(angle(A2%).data(0).poi(0), _
                  angle(A1%).data(0).poi(1), _
                    angle(A1%).data(0).poi(2), "", 0)
         If A% > 0 Then '两角和是锐角
            A1% = A%
            A2% = 0
            para2 = "0"
         ElseIf A% < 0 Then
            ...
            para2 = "0"
            v$ = add_string(v$, time_string(para1, "360", True, False), True, False)
         End If
        add_two_angle = True
        ElseIf angle(A1%).data(0).te(0) <> angle(A2%).data(0).te(1) Then
            A% = angle_number(angle(A2%).data(0).poi(0), _
                  angle(A1%).data(0).poi(1), _
                    angle(A1%).data(0).poi(2), "", 0)
           If A% > 0 Then
              A1% = A%
              A2% = 0
              v$ = minus_string(v$, time_string(para1, "180", True, False), True, False)
              para2 = "0"
           ElseIf A% < 0 Then
              A1% = -A%
              A2% = 0
      ...
      End If
End Function
Public Function minus_two_angle(A1%, A2%, _
                para1 As String, para2 As String, v$, line_direction As Byte) As Boolean
Dim A%
Dim point_dr(1) As Byte
...
End If
       ...
       If angle(A1%).data(0).line_no(line_direction) = angle(A2%).data(0).line_no(line_direction) Then '同向边一致
          If angle(A1%).data(0).te(line_direction) = angle(A2%).data(0).te(line_direction) Then '同向边一致
            A% = sign_ * angle_number(angle(A2%).data(0).poi(point_dr(1)), _
                  angle(A1%).data(0).poi(1), _
                    angle(A1%).data(0).poi(point_dr(1)), "", 0)
             If A% > 0 Then
              A1% = A%
              A2% = 0
              para2 = "0"
          ...
          Else
            ...
            If A% > 0 Then
             A1% = A%
             A2% = 0
             v$ = add_string(v$, time_string(para1, "180", True, False), True, False)
             para2 = "0"
            Else
              A1% = -A%
             A2% = 0
