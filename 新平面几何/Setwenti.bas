...
Else
   ...
   Call add_point_to_m_circle(p%, c%, temp_record, 1)
Call Wenti_form.Picture1.SetFocus
End If
End If
End Sub
...
Public Sub set_wenti_cond_22_23(ByVal p0%, ByVal p1%, ByVal p2%, _
    ByVal l1%, ByVal l2%, ByVal p5%, ty As Integer, w_n%)
'-22 过□点平行□□的直线交□□于□
'-23过□点垂直□□的直线交□□于□
'Dim tl(1) As Integer
Dim tl(1) As Integer
...
Call add_condition_to_record(wenri_, w_n%, temp_record.record_data.data0.condition_data)
Call add_point_to_line(t_p%, tl%, 0, False, False, 0, temp_record)
operate_step(C_display_wenti.m_last_input_wenti_no).last_point = last_conditions.last_cond(1).point_no
 draw_wenti_no = C_display_wenti.m_last_input_wenti_no
End Sub
...
Public Sub set_wenti_cond_33_44(ByVal p0%, _
      ByVal c%, ty As Integer, ByVal p4%, tangent_line_no%, w_n%) '作切线
...
Public Sub set_wenti_cond5_15(ByVal p0%, p2%, ByVal p1%, ByVal c%, ty%, w_n%)
...
' m_poi(p2%).data(0).parent.co_degree = 2
  ...
  '***************************************************************************
    ...
    temp_record.record_.display_no = w_n%
    Call set_mid_point(p0%, p2%, p1%, 0, 0, 0, 0, 0, temp_record, 0, 0, 0, 0, 0)
  '**********************************************************************************
...
End If
  ...
  Call C_display_wenti.set_m_inner_poi(w_n%, p2%, 3)
   ...
   temp_record.record_data.data0.condition_data.condition(1).ty = wenti_cond_
   temp_record.record_data.data0.condition_data.condition(1).no = w_n%
  Call add_point_to_m_circle(p3%, cir%, temp_record, 1)
  'call add_point_to_line(p3%,l%,0,true
 ' Call C_display_wenti.set_m_inner_point_type(w_n%, ty)
...
Public Sub set_wenti_cond10_16(ByVal p0%, ByVal p1%, _
  ByVal p2%, c%, ByVal p5%, cond_no%, paral_or_verti As Integer, ByVal ty As Integer, w_n%)
...
Dim t_coord As POINTAPI
 ...
 Call C_display_wenti.set_m_inner_circ(w_n%, c%, 1)
   ...
   temp_record.record_data.data0.condition_data.condition(1).ty = wenti_cond_
   temp_record.record_data.data0.condition_data.condition(1).no = w_n%
 Call add_point_to_m_circle(p1%, c%, record0, 1)
...
Public Sub set_wenti_cond_2(ByVal p4%, ByVal p5%, ByVal c1%, _
         ByVal c2%, ByVal line_no%, ByVal tangent_line_no%, w_n%) 'tl%,切线,=6 两园相切
...
Call add_point_to_m_circle(p4%, c1%, record0, 1)
Call add_point_to_m_circle(p5%, c2%, record0, 1)
End If
...
'-64 ⊙□□□和⊙□[down\\(_)]相切于点□
Dim p(1) As Integer
Dim l%, m%, i%, w_n%, t_l%, tan_l%
Dim circ_type(1) As Integer
Dim c_data0 As condition_data_type
Dim temp_record As total_record_type
Dim temp_tangent_line As tangent_line_type
MDIForm1.Toolbar1.Buttons(21).Image = 33
t_coord = add_POINTAPI(m_poi(tp%).data(0).data0.coordinate, _
       verti_POINTAPI(minus_POINTAPI( _
        m_Circ(c1%).data(0).data0.c_coord, m_Circ(c2%).data(0).data0.c_coord)))
tan_l% = set_tangent_line_data(m_poi(tp%).data(0).data0.coordinate, t_coord, _
           c1%, c2%, depend_condition(circle_, c1%, tp%), _
            depend_condition(circle_, c2%, tp%), 4, inner_tangent_line_by_two_circle00) '公切线
'temp_tangent_line = tangent_line(tan_l%)
'tangent_line(tan_l%) = tangent_line(last_conditions.last_cond(1).tangent_line_no + 1)
'tangent_line(last_conditions.last_cond(1).tangent_line_no + 1) = temp_tangent_line
't_l% = line_number(tp%, 0, m_poi(tp%).data(0).data0.coordinate, t_coord, _
                               depend_condition(circle_, c1%), _
                               depend_condition(circle_, c2%), _
                               aid_condition, fill_color, 0, 0)
t_l% = read_tangent_line(m_poi(tp%).data(0).data0.coordinate, 0, _
                    m_poi(tp%).data(0).data0.coordinate, inner_tangent_line_by_two_circle00, _
                      True, 0, aid_condition, fill_color)
'last_conditions.last_cond(1).tangent_line_no = last_conditions.last_cond(1).tangent_line_no + 1
'm_lin(t_l%).data(0).tangent_line_no = last_conditions.last_cond(1).tangent_line_no
'tangent_line(last_conditions.last_cond(1).tangent_line_no).data(0).line_no = t_l%
Call set_dverti(t_l%, line_number0( _
                      m_Circ(c1%).data(0).data0.center, m_Circ(c2%).data(0).data0.center, _
                       0, 0), temp_record, 0, 0, False)
't_l% = read_tangent_line(m_poi(tp%).data(0).data0.coordinate, 0, _
                    m_poi(tp%).data(0).data0.coordinate, inner_tangent_line_by_two_circle21, False)  ', aid_condition, 0, True)
record_0.data0.condition_data.condition_no = 0
Call add_point_to_line(tp%, m%, 0, no_display, False, 0, temp_record)
...
record_0.data0.condition_data.condition_no = 0
Call add_point_to_line(tp%, l%, 0, no_display, True, 0, temp_reccord)
End If
MDIForm1.Toolbar1.Buttons(21).Image = 33
...
record_0.data0.condition_data.condition_no = 0
Call add_point_to_line(tp%, m%, 0, no_display, True, 0, temp_record)
...
Call C_display_wenti.set_m_point_no(w_n%, p(1), 16, True)
   ...
   temp_record.record_data.data0.condition_data.condition(1).ty = wenti_cond_
   temp_record.record_data.data0.condition_data.condition(1).no = w_n%
 Call add_point_to_m_circle(tp%, c1%, temp_record, 1)
 Call add_point_to_m_circle(tp%, c2%, temp_record, 1)
operate_step(C_display_wenti.m_last_input_wenti_no).last_point = last_conditions.last_cond(1).point_no
draw_wenti_no = C_display_wenti.m_last_input_wenti_no
End Sub
...
If n% = -6 Then
  ...
  ' value_circle_no% = m_circle_number(1, p1%, m_poi(p1%).data(0).data0.coordinate, 0, 0, 0, _
   ...
   temp_record.record_data.data0.condition_data.condition(1).ty = wenti_cond_
   temp_record.record_data.data0.condition_data.condition(1).no = w_n%
 Call add_point_to_m_circle(p2%, value_circle_no%, record0, 1)
 'If value$ <> "" Then '未输入长度值
 'Call C_display_wenti.set_m_inner_circ(w_n%, c%, 1) '
 'Call C_display_wenti.set_m_inner_poi(w_n%, p1%, 1) '
 '  Call C_display_wenti.set_m_point_no(w_n%, _
...
Else
 ...
 End If
    Call add_condition_to_record(wenti_, w_n%, temp_record.record_data.data0.condition_data)
    Call add_point_to_line(midpoint%, line_number(p1%, p2%, _
                           pointapi0, pointapi0, _
                           depend_condition(point_, p1%), _
                           depend_condition(point_, p2%), _
                           condition, condition_color, 1, 0), 0, False, False, 0, temp_record)
    Call C_display_wenti.set_m_inner_poi(w_n%, p3%, 1)
    Call C_display_wenti.set_m_inner_poi(w_n%, midpoint%, 2)
    Call C_display_wenti.set_m_inner_poi(w_n%, p1%, 3)
    Call C_display_wenti.set_m_inner_poi(w_n%, p2%, 4)
...
Public Sub from_draw_to_input(ByVal ty As Integer, ByVal p1%, _
                        ele1 As condition_type, ele2 As condition_type, _
                          tangent_type As Integer, is_no_need_pre_input As Boolean, input_draw_step As Integer)
    ...
    m_temp_line_for_input.data(0).poi(1) = p1%
     ...
     l% = temp_line(draw_line_no)
             ...
             m_temp_line_for_input.data(1) = m_lin(0).data(0).data0
              ...
              Call from_draw_to_input(ty, p1%, ele1, ele2, tangent_type, is_no_need_pre_input, input_draw_step) '重新调用，建立输入语句
             End If
 ElseIf m_temp_circle_for_input.is_using And ((list_type_for_draw = 1 And input_draw_step = 1) Or _
              (list_type_for_draw = 2 And input_draw_step = 2)) Then
...
Select Case operator
  ...
  Case "draw_point_and_line"
           ...
           ElseIf list_type_for_draw = 2 And input_draw_step >= 1 Then '中点
              Call set_wenti_cond5_15(temp_point(0).no, 0, temp_point(1).no, 0, 5, 0)
           ElseIf list_type_for_draw = 3 And input_draw_step = 1 Then '定比 '
              Call set_wenti_cond6_6(6, temp_point(0).no, temp_point(1).no)
           ElseIf list_type_for_draw = 4 And input_draw_step >= 1 Then '定长
               Call set_wenti_cond_6_42_43_57(temp_point(0).no, temp_point(1).no, ele1, ele2, 0)
           ElseIf list_type_for_draw = 5 And input_draw_step = 3 Then '等长
           ...
           ElseIf list_type_for_draw = 6 And input_draw_step = 4 Then '画角平分线
           ...
           End If
   Case "draw_circle"
           If (list_type_for_draw = 1 And input_draw_step = 1) Or (list_type_for_draw = 2 And input_draw_step = 2) Then
                    Call set_wenti_cond7(temp_circle(0), temp_point(input_draw_step).no)
           ElseIf list_type_for_draw = 3 And input_draw_step = 4 Then
                    ...
                    End If
           ElseIf list_type_for_draw = 4 And input_draw_step = 6 Then
                    If ele1.ty = line_ Then
                     Call set_wenti_cond_2_33_44(m_lin(ele1.no).data(0).tangent_line_no)
                    ElseIf ele2.ty = line_ Then
                     Call set_wenti_cond_2_33_44(m_lin(ele2.no).data(0).tangent_line_no)
           ...
           End If
   Case "paral_verti"
      If list_type_for_draw = 1 And input_draw_step = 3 Then '平行垂直
        Call set_wenti_cond2_3(temp_point(3).no, temp_point(0).no, temp_point(1).no, temp_point(2).no, paral_or_verti, 0)
      ElseIf list_type_for_draw = 2 And input_draw_step = 2 Then '垂直平分线
        'Call set_wenti_cond4(temp_point(0).no, temp_point(1).no, temp_point(2).no, temp_point(3).no, temp_line(0), 0)
       Call set_wenti4_53_54(ty, p1%)
      Else
       Call set_wenti_cond0(p1%, ty, 0, is_no_need_pre_input)
        'Call set_wenti4_53_54(ty, p1%)
      End If
   Case "epolygon"
      If list_type_for_draw = 1 And draw_step = 2 Or _
          list_type_for_draw = 2 And draw_step = 3 Or _
           list_type_for_draw = 3 And draw_step = 4 Or _
            list_type_for_draw = 4 And draw_step = 5 Then
...
ElseIf ty = new_point_on_circle_circle12 Or _
         ty = new_point_on_circle_circle21 Then
 ...
 ElseIf tangent_line(tangent_line_no%).data(0).circ(0) > 0 Then
    Call set_wenti_cond_33_44(tangent_line(tangent_line_no%).data(0).poi(0), _
         tangent_line(tangent_line_no%).data(0).circ(0), tangent_line(tangent_line_no%).tangent_type, _
          tangent_line(tangent_line_no%).data(0).poi(1), tangent_line_no%, w_n%)
        tangent_line(tangent_line_no%).is_display_in_wenti_data = True
 ElseIf tangent_line(tangent_line_no%).data(0).circ(1) > 0 Then
     Call set_wenti_cond_33_44(tangent_line(tangent_line_no%).data(0).poi(0), _
         tangent_line(tangent_line_no%).data(0).circ(1), tangent_line(tangent_line_no%).tangent_type, _
          tangent_line(tangent_line_no%).data(0).poi(1), tangent_line(tangent_line_no%).data(0).line_no, w_n%)
        tangent_line(tangent_line_no%).is_display_in_wenti_data = True
 End If
End If
   Call add_conditions_to_record(wenti_cond_, w_n%, 0, 0, temp_record.record_data.data0.condition_data)
   Call add_point_to_m_circle(tangent_line(tangent_line_no%).data(0).poi(0), _
      tangent_line(tangent_line_no%).data(0).circ(0), temp_record, 1)
   Call add_point_to_m_circle(tangent_line(tangent_line_no%).data(0).poi(1), _
      tangent_line(tangent_line_no%).data(0).circ(1), temp_record, 1)
   Call set_tangent_line(tangent_line(tangent_line_no%).data(0).line_no, tangent_line(tangent_line_no%).data(0).poi(0), _
                                    tangent_line(tangent_line_no%).data(0).circ(0), tangent_line(tangent_line_no%).data(0).poi(1), _
                                     tangent_line(tangent_line_no%).data(0).circ(1), temp_record, tangent_line_no%, 254)
End Sub

Public Sub set_wenti4_53_54(ByVal ty As Integer, ByVal p%)
If ty = new_point_on_line Then
   Call set_wenti_cond4(temp_point(0).no, temp_point(1).no, temp_point(2).no, temp_point(3).no, temp_line(0), 0)
ElseIf ty = interset_point_line_line Then
   Call set_wenti_cond_54(temp_point(0).no, temp_point(1).no, 0, temp_point(2).no, _
                       m_poi(p%).data(0).parent.element(1).no, _
                        m_poi(p%).data(0).parent.element(2).no)
ElseIf ty = new_point_on_line_circle12 Or _
         ty = new_point_on_line_circle21 Then
       Call set_wenti_cond_53(temp_point(0).no, temp_point(1).no, temp_point(2).no, temp_point(3).no, _
               m_poi(p%).data(0).parent.element(2).no, ty)        '-53 □□的垂直平分线交⊙□(_)□
                                          '  temp_point(3).no线段的中点
                                          '-25 □□的垂直平分线交⊙□□□于□
End If
End Sub
