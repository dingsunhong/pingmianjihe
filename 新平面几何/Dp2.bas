...
Public Function change_picture(ByVal num As Integer, ByVal change_point%, Optional ByVal change_ratio_for_measure&) As Boolean
...
Case -54, -53, -25
change_picture = change_picture_54_53_25(wenti_data)
Case -52, -56
change_picture = change_picture_52_56(wenti_data)
Case -51
change_picture = change_picture_51(wenti_data)
Case -50
change_picture = change_picture_50(wenti_data, change_point%, c_p%)
Case -43
...
Case -23, -22
 change_picture = change_picture_23_22(wenti_data)
Case -18
 change_picture = change_picture_18(wenti_data)
Case -16, -12, -9, -8
 change_picture = change_picture_16_12_9_8(wenti_data)
Case -17
 change_picture = change_picture_17(wenti_data)
Case -6
 change_picture = change_picture_6(wenti_data)
Case -4
 change_picture = change_picture_4(wenti_data, change_point%, c_p%)
Case -2, -60, -59
 change_picture = change_picture_2(wenti_data)
Case -1
 change_picture = change_picture_1(wenti_data, change_point%, c_p%)
Case 0
 'change_picture = change_picture0(num)
Case 1 '线上任取一点
 change_picture = change_picture1(wenti_data, change_point%, c_p%, num)
Case 2, 3 ' 平行上任取一点
 change_picture = change_picture2_3(wenti_data)
Case 4 ' 垂直平分'　中点应记录
 change_picture = change_picture4(wenti_data, change_point%, c_p%)
Case 5 '中点
 change_picture = change_picture5(wenti_data, change_point%, c_p%)
Case 6 '　定比分点  ?????
 change_picture = change_picture6(wenti_data, change_point%, c_p%)
'If c_display_wenti.m_point_no(0) > yidian_no Then
'End If
Case 7, -61 '圆上任取一点
 change_picture = change_picture7_61(wenti_data)
'Case 8, -71
 'change_picture = change_picture8_71(wenti_data, num, change_point%, c_p%)
Case 9 '相交
 change_picture = change_picture9(wenti_data, change_point%, c_p%)
Case 10, 16, -62, -68 '过□垂直□□的直线交⊙□于□
 change_picture = change_picture10_16(wenti_data)
Case 11, -63
...
Dim i%
For i% = 1 To last_conditions.last_cond(1).line_no  '***
If m_lin(i%).data(0).data0.visible = 1 And m_lin(i%).data(0).is_change Then
...
Case -43, -42
Call draw_picture_43_42(num, no_reduce)
Case -41 '∠□□□/∠□□□=!_~
...
Case -6
Call draw_picture_6(num, no_reduce)
Case -5
 ...
 End If
   ...
   End If
        'm_poi(p3%).data(0).in_line(0) = m_poi(p2%).data(0).in_line(0) + 1
         ...
         Call add_point_to_line(p3%, l%, 0, display, True, 0, record_0.data0.condition_data)
...
If C_display_wenti.m_point_no(num, 3) > 0 And C_display_wenti.m_point_no(num, 0) > 0 Then
 ...
 If C_display_wenti.m_point_no(num, 11) = m_lin(temp_line(2)).data(0).data0.poi(0) Then
  ...
  ' Call init_Point0(last_conditions.last_cond(1).point_no)
    temp_point(0).no = last_conditions.last_cond(1).point_no
     ...
     record_0.data0.condition_data.condition_no = 0
     Call add_point_to_line(temp_point(0).no, temp_line(2).no, 0, False, False, 0, record_0.data0.condition_data)
 ElseIf C_display_wenti.m_point_no(num, 11) = m_lin(temp_line(2)).data(0).data0.poi(1) Then
  ...
  ' Call init_Point0(last_conditions.last_cond(1).point_no)
   ...
   Call get_new_char(temp_point(0).no)
   record_0.data0.condition_data.condition_no = 0
   Call add_point_to_line(temp_point(0).no, temp_line(2), 0, False, False, 0, record_0.data0.condition_data)
  End If
...
'   Call init_Point0(last_conditions.last_cond(1).point_no)
   ...
   temp_line(0) = line_number(C_display_wenti.m_point_no(num, 11), _
      ...
      record_0.data0.condition_data.condition_no = 0
      Call add_point_to_line(temp_point(1).no, temp_line(0).no, 0, False, False, 0, record_0.data0.condition_data)
   temp_line(0) = line_number(C_display_wenti.m_point_no(num, 11), _
                              temp_point(0).no, pointapi0, pointapi0, _
                              depend_condition(point_, C_display_wenti.m_point_no(num, 11)), _
                              depend_condition(point_, temp_point(0).no), _
...
If C_display_wenti.m_point_no(num, 8) = 0 Then
  ...
  Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 8), _
             pointapi0, C_display_wenti.m_point_no(num, 11), True)
End If
End Sub
...
record_0.data0.condition_data.condition_no = 0
Call add_point_to_line(p3%, tl%, 0, display, is_draw, 0, record_0.data0.condition_data)
...
End If
      input_point_type% = read_inter_point(t_coord, ele1, _
                                     ele2, temp_point(1).no, True)
         Call set_point_no_reduce(temp_point(1).no, 0)
If C_display_wenti.m_no(num) = -32 And input_point_type% = new_point_on_line_line And (ele1.no = temp_line(0) Or _
...
Call add_point_to_line(last_conditions.last_cond(1).point_no, _
   ...
   C_display_wenti.m_point_no(num, 5), 0, 0), 0, _
    no_display, True, 0, record_0.data0.condition_data)
End Sub
 ...
 '         para = time_string(para, t_para, True, False)
           Call read_multi_item(w_n%, s%, i% - 1, item, para, 0, set_v)
            If item.sig = "~" And l% = 2 Then
               ...
               item.poi(3) = p1_(1)
               Else
               Call exchang(item.n(2), item.n(3))
               item.poi(2) = p1_(1)
               item.poi(3) = p1_(0)
               End If
            End If
...
Else
 ...
 If is_four_point_on_circle(C_display_wenti.m_point_no(num, 0), _
      C_display_wenti.m_point_no(num, 1), C_display_wenti.m_point_no(num, 2), _
       C_display_wenti.m_point_no(num, 3), tn%, -1000, 0, 0, 0, 0, 0, _
         0, 0, 0, 0, _
...
Call set_line_value(C_display_wenti.m_point_no(num, 0), _
 C_display_wenti.m_point_no(num, 1), value1, 0, 0, 0, temp_record, 0, 3, False)
 Exit Sub
End If
End Sub
 ...
 Call get_new_char(last_conditions.last_cond(1).point_no)
  Call C_display_wenti.set_m_point_no(num, _
   ...
   Call add_point_to_line(last_conditions.last_cond(1).point_no, temp_line(1), 0, no_display, _
        True, 0, record_0.data0.condition_data)
last_conditions.last_cond(1).point_no = last_conditions.last_cond(1).point_no + 1
MDIForm1.Toolbar1.Buttons(21).Image = 33
  ' Call init_Point0(last_conditions.last_cond(1).point_no)
 Call get_new_char(last_conditions.last_cond(1).point_no)
...
'Call put_name(p%)
record_0.data0.condition_data.condition_no = 0
Call add_point_to_line(p%, l1%, 0, no_display, True, 0, record_0.data0.condition_data)
Call add_point_to_line(p%, l2%, 0, no_display, True, 0, record_0.data0.condition_data)
...
If tn(0, 0) > tn(0, 1) Then
 Call exchang(tn(0, 0), tn(0, 1))
End If
If tn(1, 0) > tn(1, 1) Then
 Call exchang(tn(1, 0), tn(1, 1))
End If
If tn(2, 0) > tn(2, 1) Then
 Call exchang(tn(2, 0), tn(2, 1))
End If
If tl(0) > tl(1) Then
Call exchang(tl(0), tl(1))
Call exchang(tn(0, 0), tn(1, 0))
Call exchang(tn(0, 1), tn(1, 1))
End If
If tl(1) > tl(2) Then
Call exchang(tl(1), tl(2))
Call exchang(tn(1, 0), tn(2, 0))
Call exchang(tn(1, 1), tn(2, 1))
End If
If tl(0) > tl(1) Then
Call exchang(tl(0), tl(1))
Call exchang(tn(0, 0), tn(1, 0))
...
End If
If tl(2) > tl(3) Then
Call exchang(tl(2), tl(3))
Call exchang(tn(2, 0), tn(3, 0))
Call exchang(tn(2, 1), tn(3, 1))
End If
If tl(0) > tl(1) Then
Call exchang(tl(0), tl(1))
Call exchang(tn(0, 0), tn(1, 0))
...
Next i%
End Sub
Public Sub draw_equal_angle(ByVal p1%, ByVal p2%, ByVal p3%, _
                    p_coord1 As POINTAPI, p_coord2 As POINTAPI, _
                     p0%, p4%, ByVal l_no%, ByVal C_no%, ty As Integer)
Dim r(2) As Long
Dim A!
Dim A_(1) As Long
Dim t_coord(1) As POINTAPI
r(0) = distance_of_two_POINTAPI(m_poi(p1%).data(0).data0.coordinate, _
                           m_poi(p3%).data(0).data0.coordinate)
r(2) = distance_of_two_POINTAPI(m_poi(p1%).data(0).data0.coordinate, _
                           m_poi(p2%).data(0).data0.coordinate)
r(1) = distance_of_two_POINTAPI(m_poi(p3%).data(0).data0.coordinate, _
                           m_poi(p2%).data(0).data0.coordinate)
...
End If
If l_no% > 0 Then
   Call inter_point_line_line1(m_poi(p2%).data(0).data0.coordinate, p_coord1, _
...
End If
End Sub
Public Function line_sp_angle(ByVal p1%, ByVal p2%, ByVal p3%, coord As POINTAPI, Optional ByVal l1% = 0, Optional ByVal l2% = 0) As Integer
Dim r1, r2 As Long
Dim r As Single
...
If r1 = 0 Or r2 = 0 Then
  coord = m_poi(p2%).data(0).data0.coordinate
   Exit Function
End If
r = r1 / (r1 + r2) '按比例计算角平分线与底边交点的坐标
coord = add_POINTAPI(m_poi(p1%).data(0).data0.coordinate, _
           time_POINTAPI_by_number(minus_POINTAPI _
             (m_poi(p3%).data(0).data0.coordinate, m_poi(p1%).data(0).data0.coordinate), r))
             If l1% = 0 Then
              l1% = line_number0(p1%, l2%, 0, 0)
             End If
             If l2% = 0 Then
              l2% = line_number0(p2%, p3%, 0, 0)
             End If
                     line_sp_angle = line_number(p2%, 0, m_poi(p2%).data(0).data0.coordinate, coord, _
                               depend_condition(line_, l1%), _
                               depend_condition(line_, l2%), _
                               aid_condition, fill_color, 0, 0)
End Function

Public Function inter_point_line_sp_angle_with_line(ByVal p1%, _
     ByVal p2%, ByVal p3%, ByVal p4%, ByVal p5%, coord As POINTAPI, is_change As Boolean) As Boolean
If line_sp_angle(p1%, p2%, p3%, t_coord) > 0 Then
   If line_number0(p1%, p3%, 0, 0) = line_number0(p4%, p5%, 0, 0) Then
      coord = t_coord
   Else
   inter_point_line_sp_angle_with_line = _
     inter_point_line_line1(m_poi(p2%).data(0).data0.coordinate, t_coord, m_poi(p4%).data(0).data0.coordinate, _
...
If line_sp_angle(C_display_wenti.m_point_no(num, 2), C_display_wenti.m_point_no(num, 0), _
      C_display_wenti.m_point_no(num, 3), coord) > 0 Then
 '     last_set_point = last_set_point + 1
 '      temp_point(6) = 100 - last_set_point
 '      Call set_set_point(temp_point(6))
       temp_point(6).no = 0
  ...
  'Call put_name(C_display_wenti.m_point_no(num,1))
   ...
   Call set_line_visible(C_display_wenti.m_point_no(num, 5), 1)
    temp_line(1) = line_number(temp_point(0).no, C_display_wenti.m_point_no(num, 0), _
                               ...
                               condition, condition_color, 1, 0)
 End If
          A! = sqr((m_poi(C_display_wenti.m_point_no(num, 2)).data(0).data0.coordinate.X - _
             m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.X) ^ 2 + _
               (m_poi(C_display_wenti.m_point_no(num, 2)).data(0).data0.coordinate.Y - _
                m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.Y) ^ 2)
          A! = A! + sqr((m_poi(C_display_wenti.m_point_no(num, 3)).data(0).data0.coordinate.X - _
             m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.X) ^ 2 + _
               (m_poi(C_display_wenti.m_point_no(num, 3)).data(0).data0.coordinate.Y - _
                m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.Y) ^ 2)
          A! = sqr((m_poi(C_display_wenti.m_point_no(num, 1)).data(0).data0.coordinate.X - _
              m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.X) ^ 2 + _
               (m_poi(C_display_wenti.m_point_no(num, 1)).data(0).data0.coordinate.Y - _
                m_poi(C_display_wenti.m_point_no(num, 0)).data(0).data0.coordinate.Y) ^ 2) / A!
...
Call C_display_wenti.set_m_point_no(num, last_conditions.last_cond(1).point_no, 5, True)
 ...
 Call add_point_to_line(C_display_wenti.m_point_no(num, 5), _
    line_number(C_display_wenti.m_point_no(num, 3), _
                ...
                depend_condition(point_, C_display_wenti.m_point_no(num, 3)), _
                depend_condition(point_, C_display_wenti.m_point_no(num, 4)), _
                condition, condition_color, 1, 0), 0, display, True, 0, record_0.data0.condition_data)
End If
End If
End Sub
...
End If
  Call add_point_to_m_circle( _
            C_display_wenti.m_point_no(num, 5), pointapi0, c%, True)
'******************
End Sub

Public Sub draw_picture_43_42(num As Integer, ByVal no_reduce As Byte)
Dim i%, j%, k%
Dim ele1 As condition_type
Dim ele2 As condition_type
Dim value As String
...
End If
Next i%
If draw_free_point(C_display_wenti.m_point_no(num, 3), _
    C_display_wenti.m_condition(num, 3)) Then
    Exit Sub
End If
   If i% <> 0 Then
    Call change_point_degree(C_display_wenti.m_point_no(num, 3), -3)
   End If '点poi(c_display_wenti.m_point_no(num,i%))参加推理
If C_display_wenti.m_no(num) = -42 Then
...
Public Sub draw_picture_6(ByVal num As Integer, ByVal no_reduce As Byte)
'-6 □□=!_~
'6□是线段□□上分比为!_~的分点
Dim i%
Dim value1 As String
...
Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 0), pointapi0, _
       m_circle_number(1, C_display_wenti.m_point_no(num, 2), pointapi0, _
        0, 0, 0, 0, 0, 0, 1, 1, condition, condition_color, False), True)
Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 1), pointapi0, _
       m_circle_number(1, C_display_wenti.m_point_no(num, 2), pointapi0, _
        0, 0, 0, 0, 0, 0, 1, 1, condition, condition_color, False), True)
End If
End Sub
...
Call inter_point_line_circle2(line_number0( _
 ...
 Call add_point_to_line(C_display_wenti.m_point_no(num, 0), _
        C_display_wenti.m_point_no(num, 10), 0, display, _
         True, 0, record_0.data0.condition_data)  'c_display_wenti.m_point_no(num,5) = temp_point(0)
Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 0), pointapi0, _
                      C_display_wenti.m_point_no(num, 12), True)
 ...
 Call inter_point_circle_circle2(m_Circ(C_display_wenti.m_point_no(num, 5)).data(0).data0, _
        m_Circ(C_display_wenti.m_point_no(num, 6)).data(0).data0, _
          m_Circ(C_display_wenti.m_point_no(num, 6)).data(0).data0.in_point(j%), _
           t_coord, C_display_wenti.m_point_no(num, 0))
Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 0), pointapi0, _
             C_display_wenti.m_point_no(num, 5), True) 'c_display_wenti.m_point_no(num,5) = temp_point(0)
 Call add_point_to_m_circle(C_display_wenti.m_point_no(num, 0), pointapi0, _
                  C_display_wenti.m_point_no(num, 6), True)
   'Call draw_point(Draw_form, poi(C_display_wenti.m_point_no(num,0)), 0, display)
      Exit Sub
     ...
     Call set_point_visible(last_conditions.last_cond(1).point_no, 1, False)
       ...
       record_0.data0.condition_data.condition_no = 0
          Call add_point_to_line(last_conditions.last_cond(1).point_no, C_display_wenti.m_point_no(num, 4), _
           0, display, True, 0, record_0.data0.condition_data)
'Else
'***********************
draw_picture1_mark1:
   event_statue = wait_for_draw_point
...
If solut_2order_equation(ts(2), ts(3), ts(4), ts(0), ts(1), False) Then
   If tp(1) > 0 And tp(3) > 0 Then
          ...
          Call set_line_value(tp(0), tp(1), ts(0), 0, 0, 0, temp_record, 0, 0, False)
          Call set_line_value(tp(2), tp(3), ts(1), 0, 0, 0, temp_record, 0, 0, False)
          Else
          Call set_line_value(tp(0), tp(1), ts(1), 0, 0, 0, temp_record, 0, 0, False)
          Call set_line_value(tp(2), tp(3), ts(0), 0, 0, 0, temp_record, 0, 0, False)
          End If
   Else
   End If
Else
...
temp_record = re
If it.poi(1) > 0 Then
 input_data_from_item = set_line_value(it.poi(0), it.poi(1), val, 0, 0, 0, temp_record, 0, 0, False)
...
Private Function change_picture_54_53_25(wenti_data As wentitype) As Boolean
'-53 □□的垂直平分线交⊙□[down\\(_)]于□
'-54□□的垂直平分线交□□于□
'-25 □□的垂直平分线交⊙□□□于□
Dim is_ch As Boolean
Dim mid_coord As POINTAPI
  If m_poi(wenti_data.poi(3)).data(0).is_change Or _
      m_poi(wenti_data.poi(4)).data(0).is_change Then
       mid_coord = mid_POINTAPI(m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
          m_poi(wenti_data.point_no(1)).data(0).data0.coordinate)
        If wenti_data.poi(2) > 0 Then
         Call set_point_coordinate(wenti_data.poi(2), mid_coord, True)
        End If
         is_ch = True
  Else
        If wenti_data.poi(2) > 0 Then
           mid_coord = m_poi(wenti_data.poi(2)).data(0).data0.coordinate
        Else
          mid_coord = mid_POINTAPI(m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
           m_poi(wenti_data.point_no(1)).data(0).data0.coordinate)
        End If
  End If
If wenti_data.no = -53 Or wenti_data.no = -25 Then
  If is_ch Or m_Circ(wenti_data.circ(1)).data(0).is_change Or is_ch Then '圆变化
    If inter_point_line_circle3( _
        mid_coord, False, m_poi(wenti_data.poi(3)).data(0).data0.coordinate, _
         m_poi(wenti_data.poi(4)).data(0).data0.coordinate, _
          m_Circ(wenti_data.circ(1)).data(0).data0, _
           t_coord1, 0, t_coord2, 0, wenti_data.inter_set_point_type, True) Then
            change_picture_54_53_25 = True
      ...
      End If
    Else
       change_picture_54_53_25 = False
    End If
 End If
Else
 If m_lin(wenti_data.line_no(1)).data(0).is_change Or is_ch Then
   change_picture_54_53_25 = inter_point_line_line2(mid_coord, verti_, _
             m_poi(wenti_data.poi(3)).data(0).data0.coordinate, _
               m_poi(wenti_data.poi(4)).data(0).data0.coordinate, _
                 m_poi(m_lin(wenti_data.line_no(1)).data(0).data0.poi(0)). _
                   data(0).data0.coordinate, paral_, _
                 m_poi(m_lin(wenti_data.line_no(1)).data(0).data0.poi(0)). _
                   data(0).data0.coordinate, _
                 m_poi(m_lin(wenti_data.line_no(1)).data(0).data0.poi(1)). _
                   data(0).data0.coordinate, t_coord1, wenti_data.poi(1), True, False)
 End If
End If
End Function

Private Function change_picture_52_56(wenti_data As wentitype) As Boolean
'-52∠□□□的平分线交⊙□[down\\(_)]于□
'-56∠□□□的平分线交⊙□□□于□
Dim coord As POINTAPI
Dim coord1 As POINTAPI
     change_picture_52_56 = True
  If m_lin(wenti_data.line_no(3)).data(0).is_change Or _
      m_lin(wenti_data.line_no(4)).data(0).is_change Or _
       m_Circ(wenti_data.circ(1)).data(0).is_change Then
   Call inter_point_line_sp_angle_with_circle( _
      wenti_data.poi(2), wenti_data.poi(3), _
       wenti_data.poi(4), wenti_data.circ(1), _
        coord, 0, coord1, 0)
      If wenti_data.inter_set_point_type = 1 Then
       Call set_point_coordinate(wenti_data.poi(1), coord, True)
      Else
       Call set_point_coordinate(wenti_data.poi(1), coord1, True)
      End If
     End If
End Function

Private Function change_picture_51(wenti_data As wentitype) As Boolean
change_picture_51 = True
  If m_lin(wenti_data.line_no(3)).data(0).is_change Or _
      m_lin(wenti_data.line_no(4)).data(0).is_change Or _
       m_lin(wenti_data.line_no(1)).data(0).is_change Then
   Call inter_point_line_sp_angle_with_line(wenti_data.poi(2), _
    wenti_data.poi(3), wenti_data.poi(4), _
     m_lin(wenti_data.line_no(1)).data(0).data0.poi(0), _
      m_lin(wenti_data.line_no(1)).data(0).data0.poi(1), _
      t_coord, True)
       Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
     End If
End Function

Private Function change_picture_50_(wenti_data As wentitype, t_cp%) As Boolean
Dim coord(1) As POINTAPI
Dim ty(1) As Integer
ty(1) = (wenti_data.inter_set_point_type - 1) \ 2 '
ty(0) = (wenti_data.inter_set_point_type - 1) Mod 2 '
ty(0) = ty(0) + 1
      If wenti_data.no_ = -501 Then
       If m_poi(wenti_data.poi(1)).data(0).is_change = True Or _
           m_poi(wenti_data.poi(3)).data(0).is_change = True Or _
            m_poi(wenti_data.poi(4)).data(0).is_change = True Or _
             m_poi(wenti_data.line_no(1)).data(0).is_change = True Or _
              m_poi(wenti_data.circ(1)).data(0).is_change = True Then
        Call draw_equal_angle(wenti_data.poi(1), wenti_data.poi(3), wenti_data.poi(4), coord(0), _
                coord(1), wenti_data.poi(3), wenti_data.poi(1), wenti_data.line_no(1), wenti_data.circ(1), _
                  ty(0))
       If ty(1) = 1 Then
       Call set_point_coordinate(wenti_data.poi(2), coord(0), True)
       Else
       Call set_point_coordinate(wenti_data.poi(2), coord(1), True)
       End If
       t_cp% = wenti_data.poi(2)
       change_picture_50_ = True
       End If
      Else
       If m_poi(wenti_data.poi(1)).data(0).is_change = True Or _
           m_poi(wenti_data.poi(3)).data(0).is_change = True Or _
            m_poi(wenti_data.poi(2)).data(0).is_change = True Or _
             m_poi(wenti_data.line_no(1)).data(0).is_change = True Or _
              m_poi(wenti_data.circ(1)).data(0).is_change = True Then
       Call draw_equal_angle(wenti_data.poi(1), wenti_data.poi(3), wenti_data.poi(2), coord(0), coord(1), _
                    wenti_data.poi(3), wenti_data.poi(4), wenti_data.line_no(1), wenti_data.circ(1), ty(0))
       If ty(1) = 1 Then
       Call set_point_coordinate(wenti_data.poi(4), coord(0), True)
       Else
       Call set_point_coordinate(wenti_data.poi(4), coord(1), True)
       End If
       t_cp% = wenti_data.poi(4)
       change_picture_50_ = True
       End If
      End If
End Function
...
Private Function change_picture_50(wenti_data As wentitype, ByVal change_p%, t_cp%) As Boolean
'□□是∠□□□的平分线
Dim coord As POINTAPI
Dim e!
Dim A%
'num = wenti_data.no
If wenti_data.no_ = -51 Then
   wenti_data.no = wenti_data.no_
 change_picture_50 = change_picture_51(wenti_data)
ElseIf wenti_data.no_ = -52 Then
   wenti_data.no = wenti_data.no_
 change_picture_50 = change_picture_52_56(wenti_data)
ElseIf wenti_data.no_ = -501 Or wenti_data.no_ = -502 Then
  change_picture_50 = change_picture_50_(wenti_data, t_cp%)
Else
change_picture_50 = True
  If m_poi(wenti_data.poi(1)).data(0).is_change Then
    Call is_point_in_line1(m_poi(change_p%).data(0).data0.coordinate, _
     wenti_data.poi(3), wenti_data.poi(1), _
       wenti_data.poi(3), False, t_coord, wenti_data.poi(1), 0, True)
          A% = abs_POINTAPI(minus_POINTAPI(m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
             m_poi(wenti_data.poi(3)).data(0).data0.coordinate))
          Call C_display_wenti.set_m_point_no(wenti_data.wenti_no, A%, 40, False)
          t_cp% = wenti_data.poi(1)
  ElseIf m_lin(wenti_data.line_no(2)).data(0).is_change Or _
          m_lin(wenti_data.line_no(3)).data(0).is_change Or _
           m_lin(wenti_data.line_no(4)).data(0).is_change Then
    If wenti_data.inter_set_point_type = 0 Then
    Call line_sp_angle(wenti_data.poi(2), _
          wenti_data.poi(3), _
           wenti_data.poi(4), coord)
    A% = abs_POINTAPI(minus_POINTAPI(coord, m_poi(wenti_data.poi(3)).data(0).data0.coordinate))
          e! = wenti_data.point_no(40) / A%
           t_coord = add_POINTAPI(m_poi(wenti_data.poi(3)).data(0).data0.coordinate, _
                 time_POINTAPI_by_number(minus_POINTAPI(coord, _
                  m_poi(wenti_data.poi(3)).data(0).data0.coordinate), e!))
                   Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
           t_cp% = wenti_data.poi(1)
    Else
       change_picture_50 = change_picture_50_(wenti_data, t_cp%)
...
If wenti_data.poi(1) > yidian_no Then
 If m_Circ(wenti_data.circ(1)).data(0).is_change Or _
     m_Circ(wenti_data.circ(2)).data(0).is_change Then
  If inter_point_circle_circle1(m_Circ(wenti_data.circ(1)).data(0).data0, _
      m_Circ(wenti_data.circ(2)).data(0).data0, _
...
If m_Circ(wenti_data.circ(1)).data(0).is_change Or _
     m_poi(wenti_data.point_no(0)).data(0).is_change Then
     If wenti_data.inter_set_point_type = 0 Then
     A! = wenti_data.point_no(7) / 1000
     t_coord = add_POINTAPI( _
        m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
         time_POINTAPI_by_number(verti_POINTAPI(minus_POINTAPI( _
          m_Circ(wenti_data.circ(1)).data(0).data0.c_coord, _
             m_poi(wenti_data.point_no(0)).data(0).data0.coordinate)), A!))
       'm_poi(C_display_wenti.m_point_no(0)).data(0).data0.coordinate.X _
         (m_Circ(C_display_wenti.m_point_no(42)).data(0).data0.c_coord.Y - _
           m_poi(C_display_wenti.m_point_no(0)).data(0).data0.coordinate.Y) * A!
     't_coord.Y = _
       m_poi(C_display_wenti.m_point_no(0)).data(0).data0.coordinate.Y - _
       (m_Circ(C_display_wenti.m_point_no(42)).data(0).data0.c_coord.X - _
       m_poi(C_display_wenti.m_point_no(0)).data(0).data0.coordinate.X) * A!
      Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
     ElseIf wenti_data.inter_set_point_type = 1 Then
      j% = right_triangle1(m_Circ(wenti_data.circ(1)).data(0).data0.c_coord, _
           m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
            m_Circ(wenti_data.circ(1)).data(0).data0.radii, _
              t_coord.X, t_coord.Y, 0, 0)
         Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
     ElseIf wenti_data.inter_set_point_type = 2 Then
      j% = right_triangle1(m_Circ(wenti_data.circ(1)).data(0).data0.c_coord, _
           m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
           m_Circ(wenti_data.circ(1)).data(0).data0.radii, _
           0, 0, t_coord.X, t_coord.Y)
       Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
     End If
...
If m_Circ(wenti_data.circ(1)).data(0).is_change Or _
     m_lin(wenti_data.line_no(1)).data(0).is_change Or _
...
Else '30
 If m_Circ(wenti_data.circ(1)).data(0).is_change Or _
              m_Circ(wenti_data.circ(2)).data(0).is_change Then
   Call inter_point_circle_circle1( _
     m_Circ(wenti_data.circ(1)).data(0).data0, _
      m_Circ(wenti_data.circ(2)).data(0).data0, _
       t_coord1, 0, t_coord2, 0, 0, 0, True)
  If wenti_data.inter_set_point_type = 1 Then
...
Private Function change_picture_23_22(wenti_data As wentitype) As Boolean
'-22 过□点平行□□的直线交□□于□
'-23过□点垂直□□的直线交□□于□
Dim ty As Boolean
Dim t_cond As condition_data_type
change_picture_23_22 = True
If wenti_data.poi(1) > yidian_no Then
If wenti_data.no = -22 Or wenti_data.no = 2 Then
  ty = True
Else
  ty = False
End If
If m_poi(wenti_data.point_no(0)).data(0).is_change Or _
    m_lin(wenti_data.line_no(1)).data(0).is_change Or _
     m_lin(wenti_data.line_no(3)).data(0).is_change Then
Call inter_point_line_line3(wenti_data.point_no(0), _
        ty, wenti_data.line_no(3), wenti_data.point_no(3), _
         True, wenti_data.line_no(1), t_coord, wenti_data.poi(1), True, t_cond, False)
End If
End If
End Function
...
Private Function change_picture_16_12_9_8(wenti_data As wentitype) As Boolean
'-8 □□□□□□是正六边形
'-9 □□□□□是正五边形
'-12 □□□□是正方形
'-16 △□□□是等边三角形
...
Private Function change_picture_6(wenti_data As wentitype) As Boolean
'□□=!_~
Dim r!
...
Call tangent_line_for_two_circle(wenti_data.circ(1), wenti_data.circ(2), 0)
Call set_point_coordinate(wenti_data.poi(1), _
         tangent_line(wenti_data.inter_set_point_type).p(0), True)
Call set_point_coordinate(wenti_data.poi(2), _
          tangent_line(wenti_data.inter_set_point_type).p(1), True)
For i% = 2 To 5
tangent_line(0).visible = 0
Next i%
   t_coord.X = _
      2 * m_poi(wenti_data.poi(1)).data(0).data0.coordinate.X - _
            m_poi(wenti_data.poi(2)).data(0).data0.coordinate.X
   t_coord.Y = _
      2 * m_poi(wenti_data.poi(1)).data(0).data0.coordinate.Y - _
            m_poi(wenti_data.poi(2)).data(0).data0.coordinate.Y
   Call set_point_coordinate(wenti_data.poi(3), t_coord, True)
   t_coord.X = _
       2 * m_poi(wenti_data.poi(2)).data(0).data0.coordinate.X - _
           m_poi(wenti_data.poi(1)).data(0).data0.coordinate.X
   t_coord.Y = _
       2 * m_poi(wenti_data.poi(2)).data(0).data0.coordinate.Y - _
           m_poi(wenti_data.poi(1)).data(0).data0.coordinate.Y
   Call set_point_coordinate(wenti_data.poi(4), t_coord, True)
'******
End If
End If
End Function

Private Function change_picture_1(wenti_data As wentitype, _
                                    ByVal change_p%, t_cp%) As Boolean
'□□＝□□
Dim A!
Dim p_coord(1) As POINTAPI
If wenti_data.no_ = -54 Or wenti_data.no_ = -53 Then
   wenti_data.no = wenti_data.no_
   change_picture_1 = change_picture_54_53_25(wenti_data)
ElseIf wenti_data.no_ = -31 Or wenti_data.no_ = -30 Then
   wenti_data.no = wenti_data.no_
   change_picture_1 = change_picture_31_30(wenti_data)
ElseIf wenti_data.no_ = 4 Then
   wenti_data.no = wenti_data.no_
   change_picture_1 = change_picture4(wenti_data, change_p%, t_cp%)
Else
 change_picture_1 = True
  If wenti_data.circ(1) > 0 And wenti_data.circ(2) = 0 And wenti_data.line_no(2) = 0 Then
    ' If m_Circ(wenti_data.circ(1)).data(0).is_change Then
    ' A! = m_Circ(wenti_data.circ(1)).data(0).data0.radii / dist_of_two_points( _
           m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
            m_poi(wenti_data.poi(2)).data(0).data0.coordinate)
    '      p_coord(0) = add_pointapi(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
    '        time_pointapi_by_number(minus_pointapi( _
              m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
               m_poi(wenti_data.poi(2)).data(0).data0.coordinate), A!))
    '           Call set_point_coordinate(change_p%, p_coord(0), True)
    ' End If
  Else
  If m_poi(wenti_data.poi(2)).data(0).is_change Or _
        m_lin(wenti_data.line_no(1)).data(0).is_change Or _
          m_Circ(wenti_data.circ(1)).data(0).is_change Or _
           m_Circ(wenti_data.circ(2)).data(0).is_change Then
      If wenti_data.inter_set_point_type > 0 Then
      If wenti_data.circ(1) > 0 Then
         If wenti_data.poi(3) = wenti_data.poi(1) Then
          Call inter_point_line_circle3(mid_POINTAPI( _
                m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
                 m_poi(wenti_data.poi(4)).data(0).data0.coordinate), _
                  False, m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
                 m_poi(wenti_data.poi(4)).data(0).data0.coordinate, _
                  m_Circ(wenti_data.circ(1)).data(0).data0, p_coord(0), 0, p_coord(1), 0, 0, True)
         Else
          Call inter_point_circle_circle1(m_Circ(wenti_data.circ(1)).data(0).data0, _
            m_Circ(wenti_data.circ(2)).data(0).data0, p_coord(0), 0, p_coord(1), 0, 0, 0, True)
         End If
        If wenti_data.inter_set_point_type = 1 Then
               Call set_point_coordinate(wenti_data.poi(1), p_coord(0), True)
        Else
               Call set_point_coordinate(wenti_data.poi(1), p_coord(1), True)
        End If
      ElseIf wenti_data.line_no(1) > 0 Then
        If wenti_data.poi(1) = wenti_data.poi(3) Then
          Call inter_point_line_line2(mid_POINTAPI( _
                m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
                 m_poi(wenti_data.poi(4)).data(0).data0.coordinate), _
                 verti_, m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
                 m_poi(wenti_data.poi(4)).data(0).data0.coordinate, _
                m_poi(m_lin(wenti_data.line_no(1)). _
                    data(0).data0.poi(0)).data(0).data0.coordinate, _
                  paral_, m_poi(m_lin(wenti_data.line_no(1)). _
                        data(0).data0.poi(0)).data(0).data0.coordinate, _
                         m_poi(m_lin(wenti_data.line_no(1)). _
                         data(0).data0.poi(1)).data(0).data0.coordinate, _
                           p_coord(0), wenti_data.poi(1), True, False)
        Else
         Call inter_point_line_circle3( _
           m_poi(m_lin(wenti_data.line_no(1)).data(0).data0.poi(0)).data(0).data0.coordinate, _
            True, m_poi(m_lin(wenti_data.line_no(1)). _
             data(0).data0.poi(0)).data(0).data0.coordinate, _
            m_poi(m_lin(wenti_data.line_no(1)). _
             data(0).data0.poi(1)).data(0).data0.coordinate, _
            m_Circ(wenti_data.circ(2)).data(0).data0, p_coord(0), 0, p_coord(1), 0, 0, True)
            If wenti_data.inter_set_point_type = 1 Then
               Call set_point_coordinate(wenti_data.poi(1), p_coord(0), True)
            Else
               Call set_point_coordinate(wenti_data.poi(1), p_coord(1), True)
         ...
         End If
      End If
     Else 'wenti_data.inter_set_point_type =0 Then
        A! = m_Circ(wenti_data.circ(2)).data(0).data0.radii / distance_of_two_POINTAPI( _
           m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
            m_poi(wenti_data.poi(2)).data(0).data0.coordinate)
        p_coord(0) = add_POINTAPI(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
            time_POINTAPI_by_number(minus_POINTAPI( _
...
Private Function change_picture1(wenti_data As wentitype, ByVal change_p%, t_cp%, ByVal num As Integer) As Boolean
'直线□□上任取一点□
Dim tp%
Dim A!
...
A! = wenti_data.point_no(7) / 1000
If m_poi(wenti_data.poi(2)).data(0).is_change Or _
      m_poi(wenti_data.poi(3)).data(0).is_change Or _
       m_poi(wenti_data.poi(4)).data(0).is_change Then
 If wenti_data.inter_set_point_type = 0 Then
  If wenti_data.no = 2 Then
    p_coord(0) = add_POINTAPI(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
                 time_POINTAPI_by_number(minus_POINTAPI( _
                  m_poi(wenti_data.poi(3)).data(0).data0.coordinate, _
                   m_poi(wenti_data.poi(4)).data(0).data0.coordinate), A!))
      Call set_point_coordinate(wenti_data.poi(1), p_coord(0), True)
 Else
    p_coord(0) = add_POINTAPI(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
  ...
  End If
 Else
  If wenti_data.line_no(1) > 0 Then
     Call inter_point_line_line3(wenti_data.poi(2), ty, wenti_data.line_no(4), _
            m_lin(wenti_data.line_no(1)).data(0).data0.poi(0), True, wenti_data.line_no(1), _
             p_coord(0), 0, True, t_cond, False)
        Call set_point_coordinate(wenti_data.poi(1), p_coord(0), True)
  ElseIf wenti_data.circ(1) > 0 Then
     Call inter_point_line_circle3(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
            ty, m_poi(wenti_data.poi(3)).data(0).data0.coordinate, _
                 m_poi(wenti_data.poi(4)).data(0).data0.coordinate, _
                  m_Circ(wenti_data.circ(1)).data(0).data0, _
 ...
 ElseIf m_poi(wenti_data.poi(1)).data(0).is_change Then
  If wenti_data.no = 2 Then
   Call is_point_in_line1(m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
     wenti_data.poi(3), wenti_data.poi(4), _
        wenti_data.poi(2), False, p_coord(0), wenti_data.poi(1), A!, True)
  ElseIf wenti_data.no = 3 Then
   Call is_point_in_line1(m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
     wenti_data.poi(3), wenti_data.poi(4), _
       wenti_data.poi(2), True, p_coord(0), wenti_data.poi(1), A!, True)
  End If
      Call C_display_wenti.set_m_point_no(wenti_data.wenti_no, Int(A! * 1000), 7, False)
 End If
End Function
Private Function change_picture4(wenti_data As wentitype, ByVal change_p%, t_cp%) As Boolean
'4在□□的垂直平分线上任取一点□
Dim m%
Dim A!
Dim r&
...
If wenti_data.no_ = -54 Or wenti_data.no_ = -53 Then
   wenti_data.no = wenti_data.no_
  change_picture4 = change_picture_54_53_25(wenti_data)
Else
change_picture4 = True
If wenti_data.point_no(28) <= change_p% And wenti_data.point_no(29) >= change_p% Then
t_coord = minus_POINTAPI( _
       m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
           m_poi(wenti_data.point_no(1)).data(0).data0.coordinate)
r& = abs_POINTAPI(t_coord) '□□的长
 If m_poi(wenti_data.point_no(0)).data(0).is_change Or _
      m_poi(wenti_data.point_no(1)).data(0).is_change Then
      '求中点
  mid_coord = mid_POINTAPI(m_poi(wenti_data.point_no(0)).data(0).data0.coordinate, _
...
'*********************************************************************************
   t_coord.X = _
     mid_coord.X + _
       (m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.Y - _
        m_poi(wenti_data.point_no(0)).data(0).data0.coordinate.Y) * _
         C_display_wenti.m_point_no(wenti_data.wenti_no, 11) / 1000
   t_coord.Y = _
     mid_coord.Y - _
       (m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.X - _
        m_poi(wenti_data.point_no(0)).data(0).data0.coordinate.X) * _
    ...
    '计算交点坐标
 ElseIf m_poi(wenti_data.poi(1)).data(0).is_change Then
    Call is_point_in_line1( _
         m_poi(wenti_data.poi(1)).data(0).data0.coordinate, _
           wenti_data.point_no(0), _
            wenti_data.point_no(1), _
              wenti_data.poi(2), _
                True, t_coord, wenti_data.poi(1), A!, True)
                 t_cp% = wenti_data.poi(1)
     Call C_display_wenti.set_m_point_no(wenti_data.wenti_no, Int(1000 * A!), 11, False)
End If
End If
'***************************************
End If
...
If wenti_data.point_no(29) >= yidian_no And wenti_data.point_no(28) <= yidian_no Then
 If m_poi(wenti_data.point_no(0)).data(0).is_change Or _
      m_poi(wenti_data.point_no(1)).data(0).is_change Then
  t_coord = mid_POINTAPI(m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
        m_poi(wenti_data.poi(3)).data(0).data0.coordinate)
  Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
End If
End If
...
Private Function change_picture6(wenti_data As wentitype, _
                                  ByVal change_p%, t_cp%) As Boolean
Dim v As Single
...
If m_poi(wenti_data.point_no(1)).data(0).is_change Or _
    m_poi(wenti_data.point_no(2)).data(0).is_change Then
     v = value_for_draw(wenti_data.point_no(3))
 t_coord.X = _
    m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.X + _
       (m_poi(wenti_data.point_no(2)).data(0).data0.coordinate.X - _
         m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.X) / v
 t_coord.Y = _
    m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.Y + _
       (m_poi(wenti_data.point_no(2)).data(0).data0.coordinate.Y - _
         m_poi(wenti_data.point_no(1)).data(0).data0.coordinate.Y) / v
         Call set_point_coordinate(wenti_data.poi(1), t_coord, True)
End If
End If
End Function
...
Private Function change_picture9(wenti_data As wentitype, ByVal change_p%, c_p%) As Boolean
'直线□□和直线□□交于点□
Dim t_cond As condition_data_type
change_picture9 = True
If wenti_data.poi(1) > change_p% Then
 If m_lin(wenti_data.line_no(1)).data(0).is_change Or _
     m_lin(wenti_data.line_no(2)).data(0).is_change Then
     If inter_point_line_line( _
      m_lin(wenti_data.line_no(1)).data(0).data0.poi(0), _
      m_lin(wenti_data.line_no(1)).data(0).data0.poi(1), _
      m_lin(wenti_data.line_no(2)).data(0).data0.poi(0), _
      m_lin(wenti_data.line_no(2)).data(0).data0.poi(1), _
      wenti_data.line_no(1), wenti_data.line_no(2), wenti_data.poi(1), pointapi0, False, True, True) > 0 Then
       change_picture9 = True
     End If
   End If
End If
...
Private Function change_picture10_16(wenti_data As wentitype) As Boolean
...
If wenti_data.no = 10 Or wenti_data.no = -62 Or _
               wenti_data.no = 2 Then
 ty = paral_
Else
 ty = verti_
End If
...
If inter_point_line_circle1( _
      m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
  ty, m_lin(wenti_data.line_no(4)).data(0), _
       m_Circ(wenti_data.circ(1)).data(0).data0, _
        t_coord1, 0, t_coord2, 0) > 0 Then
  If wenti_data.inter_set_point_type = 1 Then
   Call set_point_coordinate(wenti_data.poi(1), t_coord1, True)
...
If m_lin(wenti_data.line_no(1)).data(0).is_change Or _
    m_Circ(wenti_data.circ(1)).data(0).is_change Then
If wenti_data.inter_set_point_type = 0 Then
Call inter_point_line_circle2(wenti_data.line_no(1), wenti_data.poi(2), _
   wenti_data.circ(1), t_coord, wenti_data.poi(1))
Else
If inter_point_line_circle1( _
   m_poi(wenti_data.poi(2)).data(0).data0.coordinate, _
    paral_, m_lin(wenti_data.line_no(1)).data(0), _
      m_Circ(wenti_data.circ(1)).data(0).data0, _
       t_coord1, 0, t_coord2, 0) = 1 Then
  If wenti_data.inter_set_point_type = 1 Then
   Call set_point_coordinate(wenti_data.poi(1), t_coord1, True)
  ElseIf wenti_data.inter_set_point_type = 2 Then
   Call set_point_coordinate(wenti_data.poi(1), t_coord2, True)
...
If m_Circ(wenti_data.circ(1)).data(0).is_change Or _
    m_Circ(wenti_data.circ(2)).data(0).is_change Then
If wenti_data.inter_set_point_type = 0 Then
 Call inter_point_circle_circle2( _
  m_Circ(wenti_data.circ(1)).data(0).data0, _
   m_Circ(wenti_data.circ(2)).data(0).data0, _
    wenti_data.poi(2), t_coord, wenti_data.poi(1))
ElseIf wenti_data.inter_set_point_type = 1 Then
i% = inter_point_circle_circle1( _
 m_Circ(wenti_data.circ(1)).data(0).data0, _
  m_Circ(wenti_data.circ(2)).data(0).data0, _
   t_coord1, wenti_data.poi(1), _
      t_coord2, 0, 0, 0, True)
ElseIf wenti_data.inter_set_point_type = 2 Then
i% = inter_point_circle_circle1( _
 m_Circ(wenti_data.circ(1)).data(0).data0, _
  m_Circ(wenti_data.circ(2)).data(0).data0, _
   t_coord1, 0, t_coord2, wenti_data.poi(1), 0, 0, True)
      'poi(wenti_data.m_point_no(0)).data(0).is_change
      '   Call change_circle_(wenti_data.m_point_no(0), pointapi0)
End If
End If
End If
...
If wenti_data.no_ = -50 Or wenti_data.no_ = -501 Or _
    ...
    wenti_data.no_ = -50
     change_picture_4 = change_picture_50(wenti_data, change_p%, t_cp%)
...
If change_pointr% Or (wenti_data.point_no(29) <= change_pointr% And _
   ...
   ElseIf m_poi(wenti_data.poi(4)).data(0).degree = 1 Then
       If m_poi(wenti_data.poi(4)).data(0).depend_element.element(0).ty = line_ Then
          Call inter_point_line_circle3( _
       ...
       ElseIf m_poi(wenti_data.poi(4)).data(0).depend_element.element(0).ty = circle_ Then
          Call inter_point_circle_circle1( _
