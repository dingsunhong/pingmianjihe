...
Type circle_from_three_point_data
 poi(2) As Integer
  circle_no As Integer
   index As Integer
End Type
Type circle_from_three_point
poi(2) As Integer
data(8) As circle_from_three_point_data
End Type
Global Dtwo_point_line() As line_from_two_point
Global Dthree_point_() As circle_from_three_point
...
line_no(1) As Integer '¹©ËÑË÷
line_no_constr_t_angle(1) As Integer '¹¹³ÉÈ«½Ç£¬ÓÐ·½Ïò
inter_point As Integer
angle_no(1 To 4) As angle_no_type 'Ò»¸öÈ«½Ç¶ÔÓ¦4¸öÆÕÍ¨½Ç
is_used_no As Integer '¼ÇÂ¼È«½ÇÖÐ£¬´ú±í½Ç£¬ÊÇ²úÉúÈ«½ÇÊÇÊ¹ÓÃµÄ½Ç£¬Ò²ÊÇÒÔºó±í´ïÊ½ÖÐÊ¹ÓÃµÄ½Ç
index As Integer
...
circ As Integer
 poi(3) As Integer '°´µãµÄÐòºÅÅÅÁÐ£¬¹©²éÑ¯
 poi_(3) As Integer '°´µãÔÚÔ²ÉÏµÄÅÅÁÐË³Ðò£¨Ë³Ê±Õë
  angle(4) As Integer '
   angle_pair(3, 1) As Integer '£¬4 Í¬ÅÔÄÚ½Ç£¬2 ¶Ô½Ç
...
condition As Byte
k(1) As Single
initial_poi(1) As Integer
eangle_no As Integer
tangent_circle As tanget_circle_for_line
tangent_line_no As Integer
in_paral(8) As in_paral_type
degree As Byte '4 È«×ÔÓÉ,
depend_element  As depend_element_type
depend_poi(8) As Integer
in_point(10) As Integer '»­Í¼±äÉ«
in_verti(8) As in_verti_type
inform As String
line_para(2) As String 'Ò»°ãÊ½Ö±Ïß·½³Ì
is_change As Byte '=1 0r =2ÓÐÒ»µã»òÁ½µã¸Ä±ä =255ÐèÒªÖØ»­
...
Global Const two_angle_180_ = 27
...
Public Sub record_no(ByVal ty As Integer, ByVal n%, re As total_record_type, _
   init_display As Boolean, operat_type As Byte, remove_no%, Optional set_display_no = 0)
Dim i%
...
If operat_type = 0 Then
  ...
  If init_display = True And new_point(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case wenti_cond_
  ...
  re.record_data.data0.condition_data.condition(1).ty = 0
  re.record_data.data0.condition_data.condition(1).no = 0
  re.record_.display_no = n%
Case item0_
If operat_type = 0 Then
...
Case pseudo_similar_triangle_
 If operat_type = 0 Then
  ...
  re.record_ = pseudo_similar_triangle(n%).record_
   If init_display = True And pseudo_similar_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case pseudo_total_equal_triangle_
 If operat_type = 0 Then
  ...
  re.record_ = pseudo_total_equal_triangle(n%).record_
   If init_display = True And pseudo_total_equal_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case V_line_value_
 If operat_type = 0 Then
  ...
  re.record_ = V_line_value(n%).record_
   If init_display = True And V_line_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Sides_length_of_triangle(n%).record_
  If init_display = True And Sides_length_of_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
'  If init_display = True And Ddistance_of_paral_line(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
'  If init_display = True And Ddistance_of_point_line(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Sides_length_of_circle(n%).record_
  If init_display = True And Sides_length_of_circle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no _
   Then
...
If operat_type = 0 Then
 ...
 re.record_ = area_of_circle(n%).record_
  If init_display = True And area_of_circle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = area_of_element(n%).record_
  If init_display = True And area_of_element(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = two_area_of_element_value(n%).record_
  If init_display = True And two_area_of_element_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Area_of_fan(n%).record_
  If init_display = True And Area_of_fan(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dtixing(n%).record_
  If init_display = True And Dtixing(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = rhombus(n%).record_
  If init_display = True And rhombus(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dlong_squre(n%).record_
  If init_display = True And Dlong_squre(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dsqure(n%).record_
  If init_display = True And Dsqure(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = equation(n%).record_
  If init_display = True And equation(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
'   If init_display = True And Dequal_side_tixing(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case equal_side_triangle_
 ...
 re.record_ = equal_side_triangle(n%).record_
   If init_display = True And equal_side_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case equal_side_right_triangle_
 ...
 re.record_ = equal_side_right_triangle(n%).record_
   If init_display = True And equal_side_right_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case tangent_line_
 ...
 re.record_ = tangent_line(n%).record_
   If init_display = True And tangent_line(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case epolygon_
 ...
 re.record_ = epolygon(n%).record_
   If init_display And epolygon(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case equal_arc_
 ...
 re.record_ = equal_arc(n%).record_
   If init_display And equal_arc(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
Case general_string_
 ...
 re.record_ = general_string(n%).record_
   If init_display And general_string(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
'  If init_display And equal_area_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 If init_display And Darea_relation(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 If init_display And angle3_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dangle(n%).record_
  If init_display And Dangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dline1(n%).record_
  If init_display And Dline1(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Ddpoint_pair(n%).record_
   If init_display And Ddpoint_pair(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Deline(n%).record_
   If init_display And Deline(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = line3_value(n%).record_
  If init_display And line3_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = line_value(n%).record_
  If init_display And line_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = length_of_polygon(n%).record_
  If init_display And length_of_polygon(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = two_line_value(n%).record_
  If init_display And two_line_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dmid_point(n%).record_
   If init_display And Dmid_point(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dparal(n%).record_
   If init_display And Dparal(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dparallelogram(n%).record_
  If init_display And Dparallelogram(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = three_point_on_line(n%).record_
   If init_display And three_point_on_line(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = four_point_on_circle(n%).record_
   If init_display And four_point_on_circle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = three_point_on_circle(n%).record_
   If init_display And three_point_on_circle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Drelation(n%).record_
   If init_display And Drelation(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = v_Drelation(n%).record_
   If init_display And v_Drelation(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = relation_string(n%).record_
   If init_display And relation_string(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dsimilar_triangle(n%).record_
   If init_display And Dsimilar_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dtotal_equal_triangle(n%).record_
   If init_display And Dtotal_equal_triangle(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = Dverti(n%).record_
   If init_display And Dverti(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = two_order_equation(n%).record_
   If init_display And two_order_equation(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = tri_function(n%).record_
   If init_display And tri_function(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 ...
 re.record_ = verti_mid_line(n%).record_
   If init_display And verti_mid_line(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
If operat_type = 0 Then
 re.record_data = V_line_value(n%).data(0).record
 re.record_ = V_line_value(n%).record_
   If init_display And V_line_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
    V_line_value(n%).record_.display_no = 0
   End If
ElseIf operat_type = 1 Then
 V_line_value(n%).data(0).record.data0 = re.record_data.data0
...
If operat_type = 0 Then
 ...
 re.record_ = V_two_line_time_value(n%).record_
   If init_display And V_two_line_time_value(n%).record_.display_no > C_display_wenti.m_last_input_wenti_no Then
...
End Sub
Public Function set_display_string0(ByVal ty As Integer, ByVal n%, _
             concl_or_cond As Byte, conclusion_or_inform As Boolean, add_note As Boolean, _
                    is_same_theorem As Byte, ge_or_tree As Byte, dis_ty As Byte, _
                        is_depend As Boolean, Optional display_type As Boolean = True) As String '½¨Á¢ÏÔÊ¾Óï¾ä
...
If ty > 0 And n% > 0 Then
 Call record_no(ty, n%, re1, False, 0, 0)
  'Call record_no(ty, n%, re1, True, 0, 0)
  Else
  Exit Function
 End If
If re1.record_.display_no > 0 Then
  Exit Function
End If
Select Case ty
Case wenti_cond_
...
set_display_string0_gennral_string:
 ...
 If is_depend Then
  ...
  Next i%
 End If
 If tangent_line(n%).data(0).record.display_string <> "" And display_type Then
   stri(0) = tangent_line(n%).data(0).record.display_string
 Else
 ...
 If tangent_line(n%).data(0).circ(1) = 0 Then
    If display_type Then
       stri(0) = LoadResString_(1475, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                         ...
                         "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(0)).data(0).data0.my_name(0))
       tangent_line(n%).data(0).record.display_string = stri(0)
    Else
             stri(0) = LoadResString_(1475, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                                  m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(1)).data(0).data0.name + _
                         "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(0)).data(0).data0.my_name(1))
    End If
 ElseIf tangent_line(n%).data(0).circ(0) = 0 Then
  If display_type Then
   stri(0) = LoadResString_(1475, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                                  m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(1)).data(0).data0.name + _
                         "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(1)).data(0).data0.my_name(0))
    tangent_line(n%).data(0).record.display_string = stri(0)
  Else
   stri(0) = LoadResString_(1475, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                                  m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(1)).data(0).data0.name + _
                         "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(1)).data(0).data0.my_name(1))
  End If
 Else
  If display_type Then
  stri(0) = LoadResString_(1480, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                                        m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(1)).data(0).data0.name + _
                               "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(0)).data(0).data0.my_name(0) + _
                                "\\3\\" + m_Circ(tangent_line(n%).data(0).circ(1)).data(0).data0.my_name(0))
   tangent_line(n%).data(0).record.display_string = stri(0)
  Else
  stri(0) = LoadResString_(1480, "\\1\\" + m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(0)).data(0).data0.name + _
                                        m_poi(m_lin(tangent_line(n%).data(0).line_no).data(0).data0.poi(1)).data(0).data0.name + _
                               "\\2\\" + m_Circ(tangent_line(n%).data(0).circ(0)).data(0).data0.my_name(1) + _
                                "\\3\\" + m_Circ(tangent_line(n%).data(0).circ(1)).data(0).data0.my_name(1))
  End If
 End If
 Else
  If tangent_line(n%).data(0).circ(1) = 0 Then
...
Else
 ...
 ElseIf re1.record_data.data0.condition_data.condition_no < 200 Then  'ÍÆÀíÌõ¼þ
   note_string = "("
  For i% = 1 To re1.record_data.data0.condition_data.condition_no
   If i% > 1 Then
      note_string = note_string + ","
   End If
     If re1.record_data.data0.condition_data.condition(i%).ty = wenti_cond_ Then
       note_string = note_string + CStr(re1.record_data.data0.condition_data.condition(i%).no)
     Else
       Call record_no(re1.record_data.data0.condition_data.condition(i%).ty, _
             re1.record_data.data0.condition_data.condition(i%).no, re2, False, 0, 0)
      note_string = note_string + CStr(re2.record_.display_no)
     End If
  Next i%
     note_string = note_string + ")"
     '*******************************************************************
 ...
 '      note_string = LoadResString_(1655, "")  'note_string +_¸¨ÖúÁ¿£¬Ïß£¬µã
 '    Else
 '     If note_string = "(" Then
 '       note_string = ""
 '     Else
...
If ty = general_string_ Then
 ...
 Call set_display_string0(ty, n%, concl_or_cond, conclusion_or_inform, add_note, 0, ge_or_tree, dis_ty, is_depend)
...
Public Function read_string_from_string(st%, ByVal s1$, brackets1$, brackets2$, p%, S2$, s3$, _
Optional read_inside_only As Boolean = False) As String
Dim j%, k%
Dim start_p%, end_p%
Dim first_end%, third_start%
's1$ = s1$
...
If p% = 0 Then
  ...
  Exit Function
Else 'p%>0
   If read_inside_only Then 'Ö»¶ÁÀ¨ºÅÄÚ
    start_p% = p% + Len(brackets1$) '
   Else
    start_p% = p%
   End If
     first_end = p% - 1
  '*****************************************************
   'If brackets1$ = brackets2$ Then 'Á½¸öÀ¨Í¬
      j% = InStr(p% + Len(brackets1$), s1$, brackets2$, 0)
   If read_inside_only Then
      end_p% = j% - 1
   Else
      end_p% = Len(brackets2$) + j% + -1
   End If
    third_start% = j% + Len(brackets2$)
   'Else
   'k% = 1
...
read_string_from_string_mark1:
   If j% = 0 Then
     ...
     s3$ = ""
   Else
     read_string_from_string = Mid$(s1$, start_p%, end_p% - start_p% + 1)
     S2$ = Mid$(s1$, 1, first_end%)
     s3$ = Mid$(s1$, third_start%, Len(s1) - third_start% + 1)
   End If
End If
End Function
...
s1$ = ""
Do
ty = cut_string_by_string(ts$, old_s$, S2$, s3$)
If ty Then
  s1$ = s1$ + S2$ + new_string
  ts$ = s3
...
Public Function get_record_data(ByVal data_ty As Integer, _
          ByVal data_no As Integer) As record_data_type
Select Case data_ty
Case wenti_cond_
       get_record_data.data0.condition_data.condition_no = 1
       get_record_data.data0.condition_data.condition(1).ty = wenti_cond_
       get_record_data.data0.condition_data.condition(1).no = data_no
Case relation_string_
   get_record_data = relation_string(data_no).data(0).record
'Case item0_
  ' get_record_data =  item0(data_no).data(0).dis_ty, True, is_depend)
Case new_point_
  get_record_data = new_point(data_no).data(0).record
Case V_line_value_ '
    get_record_data = V_line_value(data_no).data(0).record
Case area_of_element_
    get_record_data = area_of_element(Abs(data_no)).data(0).record
Case sides_length_of_triangle_
    get_record_data = Sides_length_of_triangle(data_no).data(0).record
Case area_of_circle_
    get_record_data = area_of_circle(data_no).data(0).record
Case sides_length_of_circle_
    get_record_data = Sides_length_of_circle(data_no).data(0).record
Case area_of_fan_
    get_record_data = Area_of_fan(data_no).data(0).record
Case point_type_
   get_record_data = new_point(data_no).data(0).record
Case two_line_value_
   get_record_data = two_line_value(data_no).data(0).record
Case tixing_
   get_record_data = Dtixing(data_no).data(0).record
Case rhombus_
   get_record_data = rhombus(data_no).data(0).record
Case long_squre_
   get_record_data = Dlong_squre(data_no).data(0).record
Case Squre
   get_record_data = Dsqure(data_no).data(0).record
Case equation_
   get_record_data = equation(data_no).data(0).record
Case general_string_
   get_record_data = general_string(data_no).data(0).record
Case tangent_line_
   get_record_data = tangent_line(data_no).data(0).record
Case equal_side_triangle_
   get_record_data = equal_side_triangle(data_no).data(0).record
Case equal_side_right_triangle_
   get_record_data = equal_side_right_triangle(data_no).data(0).record
Case epolygon_
   get_record_data = epolygon(data_no).data(0).record
Case equal_arc_
   get_record_data = equal_arc(data_no).data(0).record
Case area_relation_
  get_record_data = Darea_relation(data_no).data(0).record
'**********************************************************
Case angle3_value_
   get_record_data = angle3_value(data_no).data(0).record
'**********************************************************
Case Dangle_
   get_record_data = Dangle(data_no).data(0).record
'************************************************************
Case Dline_
   get_record_data = Dline1(data_no).data(0).record
'***************************************************
Case dpoint_pair_
   get_record_data = Ddpoint_pair(data_no).data(0).record
'*******************************************************
Case eline_
   get_record_data = Deline(data_no).data(0).record
Case midpoint_ 'equal_segment_on_line
   get_record_data = Dmid_point(data_no).data(0).record
'***********************************************************
Case mid_point_line_
   get_record_data = mid_point_line(data_no).data(0).record
Case length_of_polygon_
   get_record_data = length_of_polygon(data_no).data(0).record
Case line_value_
   get_record_data = line_value(data_no).data(0).record
'**********************************************************
Case line3_value_
  get_record_data = line3_value(data_no).data(0).record
'*****************************************************
Case paral_
  get_record_data = Dparal(data_no).data(0).data0.record
'*********************************************
Case parallelogram_
  get_record_data = Dparallelogram(data_no).data(0).record
 
'***************************************************
Case point3_on_line_
   get_record_data = three_point_on_line(data_no).data(0).record
'****************************************************************
Case point4_on_circle_
   get_record_data = four_point_on_circle(data_no).data(0).record
Case relation_
   get_record_data = Drelation(data_no).data(0).record
'****************************************************************
Case similar_triangle_
   get_record_data = Dsimilar_triangle(data_no).data(0).record
'****************************************************
Case tri_function_
   get_record_data = tri_function(data_no).data(0).record
'*************************************************************
Case total_equal_triangle_
   get_record_data = Dtotal_equal_triangle(data_no).data(0).record
Case verti_
   get_record_data = Dverti(data_no).data(0).record
'*************************************************
Case verti_mid_line_
   get_record_data = verti_mid_line(data_no).data(0).record
 '*********************************************************
End Select
