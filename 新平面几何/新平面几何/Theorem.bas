...
Public Sub add_conditions_to_record(ByVal con_ty As Byte, _
...
tl(2) = Dmid_point(no%).data(0).data0.lin
...
End If
 ...
 tA(2) = b1%
 tA(3) = b2%
 v$ = minus_string(tv(0), tv(1), True, False)
 For i% = 0 To 1
  If s(i%) = "0" Then
     tA(i%) = 0
  End If
 ...
 Next j%
Next i%
Call remove_record_for_zero_para(s(), tA(), 5)
If s(3) = "0" Then
  ...
  If InStr(1, ts(0), "F", 0) = 0 And InStr(1, ts(1), "F", 0) = 0 Then
   If triangle_data.line_value(A2%) > 0 And _
    ...
    Th_sin = set_line_value(triangle_data.poi(A3%), _
       triangle_data.poi(A2%), ts(0), 0, 0, 0, temp_record, _
        0, no_reduce, False)
     If Th_sin > 1 Then
      Exit Function
     End If
   ElseIf triangle_data.line_value(A1%) > 0 And _
    ...
    Th_sin = set_line_value(triangle_data.poi(A3%), _
       triangle_data.poi(A1%), ts(0), 0, 0, 0, temp_record, _
        0, no_reduce, False)
     If Th_sin > 1 Then
      Exit Function
     End If
          ...
          divide_string(time_string("2", _
            area_of_element(tri_data.area_no).data(0).value, False, False), _
             line_value(n(0)).data(0).data0.value, True, False), _
              0, 0, 0, temp_record, 0, no_reduce, False) '底边
        If Th_area_of_triangle1 > 1 Then
         Exit Function
        End If
       End If
          ...
          divide_string(time_string("2", _
            area_of_element(tri_data.area_no).data(0).value, False, False), _
             line_value(tri_data.line_value(p_k1%)).data(0).data0.value, True, False), _
              0, 0, 0, temp_record, 0, no_reduce, False) '高
        If Th_area_of_triangle1 > 1 Then
         Exit Function
        End If
   End If
       ...
       s = time_string(line_value(S2).data(0).data0.value, ts, True, False)
        s = divide_string(time_string("2", _
         ...
         Th_area_of_triangle2 = set_line_value(tri_data.poi(k2%), _
           tri_data.poi(k3%), s, 0, 0, 0, temp_record, _
         0, no_reduce, False)
        If Th_area_of_triangle2 > 1 Then
         Exit Function
        End If
     ...
     If th_chose(154).chose = 1 Then
      temp_record.record_data.data0.theorem_no = 154
       ...
       th_cos_ = set_line_value(tri_data.poi(tn(1)), tri_data.poi(tn(2)), _
           ts_, 0, 0, 0, temp_record, 0, no_reduce, False)
     End If
    End If
    End If
'    if tria
...
Next i%
End Function
Public Sub add_condition_to_record(ByVal con_ty As Byte, ByVal con_no%, re As condition_data_type, is_first As Byte)
Dim j%, k%
Dim temp_record As record_data_type
If con_ty = 0 And con_no% = 0 Then
   Exit Sub
End If
...
If con_no% > 0 Then
   For j% = 1 To re.condition_no
    If con_ty = 254 And con_ty = re.condition(j%).ty Then
