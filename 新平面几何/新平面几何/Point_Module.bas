...
Public Function m_point_number(p_coord As POINTAPI, ByVal ty As Byte, ByVal visible As Byte, ByVal color As Byte, _
                           name As String, d_ele1 As condition_type, d_ele2 As condition_type, _
                            inter_ty As Integer, is_set_data As Boolean) As Integer
...
If name = "" Then
    p_data0.data0.name = next_char(p%, "", 0, 0)
    name = p_data0.data0.name
Else
    p_data0.data0.name = name
End If
p_data0.degree = 2
'If inter_ty > 0 Then
...
End If
End Sub
Public Sub new_point_no(point_no%)
Dim i%
If point_no > 0 Then '
...
Public Function add_point_to_line(ByVal p%, ByVal lin%, n%, dis As Boolean, _
          is_draw As Boolean, red_point_start%) As Byte
Dim i%, j%, t_p1%, tp%, tn%
Dim r!
Dim k%
...
'检查tp%是否在线上
  If add_point_to_line_data(p%, t_l_data, n%, dis, is_draw, red_point_start%, t_l_data) = False Then '新点插入到直线数据中，并重新设置直线的端点坐标
    Exit Function
  End If
    ...
    m_lin(lin%).data(0) = t_l_data
     For i% = 1 To m_lin(lin%).data(0).data0.in_point(0)
          If m_lin(lin%).data(0).data0.in_point(i%) <> p% Then
           '设两点线
               Call set_line_from_two_point(tp%, t_l_data.data0.in_point(i%), 0, 0, lin%, 0, "", temp_record.record_data)
          End If
    Next i%
           ...
           For j% = i% + 1 To m_lin(lin%).data(0).data0.in_point(0)
            ...
            '设三点线
             add_point_to_line = set_three_point_on_line(t_l_data.data0.in_point(i%), _
                t_l_data.data0.in_point(j%), tp%, temp_record, 0, 0, True)
             End If
           Next j%
    Next i%
...
Public Function add_point_to_line_data(ByVal p%, l_data As line_data_type, n%, dis As Boolean, is_draw As Boolean, _
                red_point_start%, out_line_data As line_data_type) As Boolean  '=f 无添加 =t添加新点
Dim i%, j%, tp%, tn%
Dim r!
Dim k%
 ...
 Next i%
         t_l.data0.in_point(0) = t_l.data0.in_point(0) + 1
            t_l.data0.in_point(t_l.data0.in_point(0)) = p%
             t_l.is_change = True '
            t_l.data0.color(t_l.data0.in_point(0)) = _
                         t_l.data0.color(t_l.data0.in_point(0) - 1)  '最后一段的颜色，与原来最后一段同
             t_l.is_change = True
            n% = t_l.data0.in_point(0)
add_point_to_line_out:
If t_l.is_change And t_l.data0.in_point(0) >= 2 Then 'And t_l.data0.poi(0) = 0 Then '设置端点
