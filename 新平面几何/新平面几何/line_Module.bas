...
Else
     l% = search_for_line_number_from_two_point(p1%, p2%, n1%, n2%) '由两点确定已有直线或建立新直线
      If l% > 0 Then 'ty1 = condition Then
       ...
       If m_lin(line_number).data(0).data0.visible <> visible Or _
            m_lin(line_number).data(0).data0.total_color <> color Then '如果原线不可示,根据visible 重新设置
          m_lin(line_number).data(0).data0.visible = visible
           m_lin(line_number).data(0).data0.total_color = color
            m_lin(line_number).data(0).is_change = True
       ElseIf m_lin(line_number).data(0).data0.type = line_type Then '可见，且颜色同，不必重画
        ...
        If m_lin(line_number).data(0).data0.type = condition And line_type = conclusion Then '原来是condition现在是conclusion重新设置
          m_lin(line_number).data(0).data0.type = conclusion
           For i% = n1% To n2% - 1 '将结论部分变为红色,负点号开始的线段为红色
            ...
            'End If
          Next i%
          For i% = 1 To m_lin(line_number).data(0).data0.in_point(0) - 1
            If m_lin(line_number).data(0).data0.color(i%) <> conclusion_color Then
              GoTo set_color_out
            End If
          Next i%
...
ElseIf p1% > 0 And p2% > 0 Then '两点序号为正
  For i% = 1 To m_poi(p1%).data(0).in_line(0) '已有过两点直线
   For j% = 1 To m_poi(p2%).data(0).in_line(0)
     If m_poi(p1%).data(0).in_line(i%) = m_poi(p2%).data(0).in_line(j%) Then
      set_line = m_poi(p1%).data(0).in_line(i%)
       If m_lin(set_line).data(0).data0.visible = 0 And visible = 1 Then
          m_lin(set_line).data(0).data0.visible = 1
           m_lin(set_line).data(0).is_change = True
            Call C_display_picture.re_draw_line(set_line)
       End If
       For k% = 1 To m_lin(set_line).data(0).data0.in_point(0)
         If m_lin(set_line).data(0).data0.in_point(k%) = p1% Then
            n1% = k%
         ElseIf m_lin(set_line).data(0).data0.in_point(k%) = p2% Then
            n2% = k%
         End If
       Next k%
...
t_data0 = Ldata0 '输入新的直线数据
   If t_data0.data0.in_point(0) < 1 Then
     Exit Sub
   End If
   If last_conditions.last_cond(1).line_no Mod 10 = 0 And line_no% > last_conditions.last_cond(1).line_no Then '增加数据库的直线数据容量_
      ReDim Preserve m_lin(last_conditions.last_cond(1).line_no + 10) As line_type
      ...
      If is_two_line_same0(m_lin(i%).data(0).data0, t_data0.data0) > 0 Then
        line_no = i%
         For j% = 1 To Ldata0.data0.in_point(0)
          Call add_point_to_line(Ldata0.data0.in_point(j%), line_no, 0, True, False, 0)
         Next j%
...
'未发现已有相同的直线
 If line_no% = 0 Then
  If last_conditions.last_cond(1).line_no Mod 10 = 0 Then
   ReDim Preserve m_lin(last_conditions.last_cond(1).line_no + 10) As line_type
  End If
