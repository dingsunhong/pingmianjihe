...
ReDim Preserve m_lin(10) As line_type
...
'Next i%
If m_lin(no1%).data(0).data0.visible >= m_lin(no2%).data(0).data0.visible Then
...
For i% = 1 To last_conditions.last_cond(1).line_no
  If i% <> no1% And i% <> no2% And i% <> replace_no% Then
    tn% = total_angle_no(replace_no%, i%)
     For j% = 0 To 3
...
For m% = 0 To 1
 For n% = 0 To 1
  If n1_(n%) > 0 Then
t_A(0).line_no(m%) = n1_(n%) '全角有一边(m%)要替换
 t_A(0).line_no((m% + 1) Mod 2) = -1
Call search_for_total_angle(t_A(0), n_(0), m%, 1)
 t_A(0).line_no((m% + 1) Mod 2) = 30000
Call search_for_total_angle(t_A(0), n_(1), m%, 1)
last_tn% = 0
 ...
 If tangent_line(i%).data(0).line_no = no1% Or tangent_line(i%).data(0).line_no = no2% Then
  ...
  If is_tangent_line(replace_no%, tangent_line(i%).data(0).poi(0), _
          tangent_line(i%).data(0).ele(0).no, tangent_line(i%).data(0).poi(1), _
           tangent_line(i%).data(0).ele(1).no, tan_l, tn%, 0, 0, record_0) Then
            If i% = tn% Then
