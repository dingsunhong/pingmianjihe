...
remove_record = True
 For i% = 0 To 1
 If search_for_total_angle(T_angle(no%).data(0), n_(0)) = False Then
     remove_record = False
 End If
 Next i%
 If remove_record Then
  For i% = 0 To 1
   For j% = n_(0) + 1 To 2 Step -1
    T_angle(j%).data(0).index = _
     T_angle(j% - 1).data(0).index
   Next j%
    T_angle(1).data(0).index = 0
Next i%
...
remove_record = True
  For i% = 0 To 1
  If search_for_angle(angle(no%).data(0), n_(i%), i%, 1, 0, T_angle(0).data(0), 0) = False Then
   remove_record = False
  End If
  Next i%
  If remove_record Then
...
End If
For i% = 0 To 3
If search_for_four_point_on_circle(four_point_on_circle(no%).data(0), n_(i%)) = False Then
remove_record = False
End If
Next i%
If remove_record Then
