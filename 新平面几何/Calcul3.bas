...
'单项
  fa$ = fs$
    s1$ = "1"
ElseIf s1$ <> "0" And S2$ <> "0" Then 'And s3$ = "0" And S4$ = "0" Then
  S2$ = divide_string(S2$, s1$, True, False)
  s3$ = divide_string(s3$, s1$, True, False)
  S4$ = divide_string(S4$, s1$, True, False)
  s1$ = "1"
...
Public Sub exchange_two_string(s1$, S2$) '交换两个基本点字符
Dim ts As String
ts = s1$
 s1$ = S2$
  S2$ = ts
