...
Global red_tangent_point As Integer
Global tangent_circle_with_red_tangent_point As Integer
...
Call add_point_to_m_circle(t_p1%, cr%, 255)
Call add_point_to_m_circle(t_p2%, cr%, 255)
Call add_point_to_m_circle(t_p3%, cr%, 255)
...
Public Function inter_point_line_line(p1%, p2%, p3%, p4%, l1%, l2%, p%, _
                                    out_coord As POINTAPI, _
                                     is_set_data As Boolean, is_draw As Boolean, _
                                      is_change As Boolean, Optional is_need_re_draw As Boolean = True) As Integer '新点=true
...
If l1% > 0 And l2% > 0 Then
    If p% = 0 Then '两直线没有交点
        p% = m_point_number(out_coord, condition, 1, condition_color, "", _
           ...
           Call add_point_to_line(p%, l1%, 0, display, is_draw, 0, temp_record, True, is_need_re_draw)
           Call add_point_to_line(p%, l2%, 0, display, is_draw, 0, temp_record, True, is_need_re_draw)
...
read_out_type = draw_new_point(T_mouse_down_coord, ele1, ele2, red, False, 0, draw_step)
 ...
 ElseIf draw_step = 1 Or draw_step = 4 Then
    ...
    read_out_type = draw_new_point(T_mouse_down_coord, ele1, ele2, red, False, 255, draw_step)
 ...
 ElseIf draw_step = 2 Or draw_step = 5 Then
         ...
         Call set_forbid_line
         Call set_forbid_circle
       If draw_new_point(mouse_down_coord, ele1, ele2, red, False, 1, draw_step) > 0 Then '读出已知点
         If find_same_point_from_two_points(m_poi(temp_point(draw_step).no).data(0).in_circle, _
...
Function read_inter_point(in_coord As POINTAPI, ele1 As condition_type, ele2 As condition_type, _
                            p%, ByVal is_set_data As Boolean, input_type As Integer, _
                              Optional need_control As Byte = 0, Optional is_need_re_draw As Boolean = True) As Integer
                           'in_coord 输入点的坐标
               ...
               If p% = 0 Then
                  ...
                  If is_set_data Then
                   read_inter_point = inter_point_line_line(0, 0, 0, 0, ele1.no, ele2.no, _
                                   p%, in_coord, is_set_data, True, False, is_need_re_draw)                       '两线交点
                    'm_poi(p%).data(0).degree = 0
             ...
             '    Call set_point_parent(line_, ele2.no, m_poi(p%).data(0))
               ...
               ElseIf p% > 0 Then '
                 ...
                 If l_p(0) = 0 And l_p(1) > 0 And is_set_data Then '落在第二条线上
                      Call add_point_to_line(p%, ele1.no, 0, True, True, 0, temp_record, True, is_need_re_draw)
...
point_on_line_circle:
           ...
           ElseIf p% > 0 Then
                 ...
                 ElseIf l_p(0) > 0 And l_p(1) = 0 And is_set_data Then '
                      ele1.no = ele2.no
                      ele1.ty = circle_
                       Call add_point_to_m_circle(p%, ele1.no, record0, 0)
                    ...
                    p% = m_point_number(t_coord1, condition, 1, condition_color, "", _
                     depend_condition(ele1.ty, ele1.no), ele2, new_point_on_line, True)
                      '‘m_poi(p%).data(0).degree = 1
                                          record_0.data0.condition_data.condition_no = 0
                       Call add_point_to_line(p%, ele1.no, 0, display, True, 0, temp_record, True, _
                         is_need_re_draw)
                        read_inter_point = new_point_on_line
                  End If
                 ElseIf p% > 0 Then
 ...
 '************************************************************8
                If p% = 0 And is_set_data Then
                  p% = m_point_number(in_coord, condition, 1, condition_color, "", ele1, ele2, read_inter_point, True)
                   m_poi(p%).data(0).degree = 0
                   Call add_point_to_m_circle(p%, ele1.no, record0, 0) '= 0 Then '已有点
                   Call add_point_to_m_circle(p%, ele2.no, record0, 0) '= 0 Then '已有点
                End If
                m_poi(p%).data(0).parent.inter_type = new_inter_point_type(m_poi(p%).data(0).parent.inter_type, read_inter_point)
      Else 'ele2=0圆上的点
           in_coord = t_coord1
...
End Sub
Public Function set_polygon(ByVal p1%, ByVal p2%, _
                            n%, polygon_no%, ByVal d As Integer, is_e As Boolean, _
                              num%) As Boolean
 'n% =3,4,5,6
Dim i%, l%, j%, tp%
Dim tl(5) As Integer
...
For i% = 1 To last_conditions.last_cond(1).epolygon_no
    If epolygon(i%).data(0).p.total_v = n Then
       For j% = 1 To n%
       If (epolygon(i%).data(0).p.v(j%) = p1% And epolygon(i%).data(0).p.v((j% + 1) Mod n%) = p2% _
             And epolygon(i%).data(0).p.direction = d) Or _
              (epolygon(i%).data(0).p.v(j%) = p2% And epolygon(i%).data(0).p.v((j% + 1) Mod n%) = p1% _
             And epolygon(i%).data(0).p.direction = -d) Then
               polygon_no% = i%
                set_polygon = False
               Exit Function
       End If
...
ReDim Preserve epolygon(last_conditions.last_cond(1).poly_no + 10) As epolygon_type
End If
last_conditions.last_cond(1).epolygon_no = last_conditions.last_cond(1).epolygon_no + 1
polygon_no% = last_conditions.last_cond(1).epolygon_no
End If
set_polygon = True
tl(0) = line_number(p1%, p2%, pointapi0, pointapi0, _
                    depend_condition(point_, p1%), depend_condition(point_, p2%), _
                    condition, condition_color, 1, 0) '画第一条边
Call set_parent(point_, p1%, epolygon_, polygon_no%, 0)
Call set_parent(point_, p2%, epolygon_, polygon_no%, 0)
tl(1) = tl(0)
'设置基点
epolygon(polygon_no%).data(0).p.direction = d
epolygon(polygon_no%).data(0).p.is_e_polygon = is_e
epolygon(polygon_no%).data(0).p.v(0) = p1%
epolygon(polygon_no%).data(0).p.v(1) = p2%
'MDIForm1.Toolbar1.Buttons(21).Image = 33
Call set_polygon2(p1%, p2%, n%, polygon_no%, False)
 '正四边形共线边合并
 'If is_e = True Then
' If epolygon(polygon_no%).data(0).p.total_v = 4 Then
 'ElseIf epolygon(polygon_no%).data(0).p.total_v = 4 Then
...
Public Sub set_polygon2(ByVal p1%, ByVal p2%, _
  ByVal n%, poly_no%, is_change As Boolean)
'无图
Dim i%, l%, j%
Dim A!
A! = PI * (n% - 2) / n%
   If p1% > 0 And p2% > 0 And epolygon(poly_no%).data(0).p.v(0) = 0 And epolygon(poly_no%).data(0).p.v(1) = 0 Then
      epolygon(poly_no%).data(0).p.v(0) = p1%
      epolygon(poly_no%).data(0).p.v(1) = p2%
   End If
   epolygon(poly_no%).data(0).p.coord(0) = m_poi(epolygon(poly_no%).data(0).p.v(0)).data(0).data0.coordinate
   epolygon(poly_no%).data(0).p.coord(1) = m_poi(epolygon(poly_no%).data(0).p.v(1)).data(0).data0.coordinate
   epolygon(poly_no%).data(0).p.total_v = n%
For i% = 2 To n% - 1
  If epolygon(poly_no%).data(0).p.direction = -1 Then
   t_coord.X = epolygon(poly_no%).data(0).p.coord(i% - 1).X + _
    (epolygon(poly_no%).data(0).p.coord(i% - 2).X - epolygon(poly_no%).data(0).p.coord(i% - 1).X) * Cos(A!) - _
     (epolygon(poly_no%).data(0).p.coord(i% - 2).Y - epolygon(poly_no%).data(0).p.coord(i% - 1).Y) * Sin(A!)
   t_coord.Y = epolygon(poly_no%).data(0).p.coord(i% - 1).Y + _
    (epolygon(poly_no%).data(0).p.coord(i% - 2).X - epolygon(poly_no%).data(0).p.coord(i% - 1).X) * Sin(A!) + _
     (epolygon(poly_no%).data(0).p.coord(i% - 2).Y - epolygon(poly_no%).data(0).p.coord(i% - 1).Y) * Cos(A!)
  Else
   t_coord.X = epolygon(poly_no%).data(0).p.coord(i% - 1).X + _
    (epolygon(poly_no%).data(0).p.coord(i% - 2).X - epolygon(poly_no%).data(0).p.coord(i% - 1).X) * Cos(A!) + _
     (epolygon(poly_no%).data(0).p.coord(i% - 2).Y - epolygon(poly_no%).data(0).p.coord(i% - 1).Y) * Sin(A!)
   ...
   (epolygon(poly_no%).data(0).p.coord(i% - 2).X - epolygon(poly_no%).data(0).p.coord(i% - 1).X) * Sin(A!) + _
     (epolygon(poly_no%).data(0).p.coord(i% - 2).Y - epolygon(poly_no%).data(0).p.coord(i% - 1).Y) * Cos(A!)
  End If
   epolygon(poly_no%).data(0).p.coord(i%) = t_coord
   Call draw_new_point(epolygon(poly_no%).data(0).p.coord(i%), depend_condition(point_, p1%), _
             depend_condition(point_, p2%), _
               False, True, 0, draw_step, False, False)
   epolygon(poly_no%).data(0).p.v(i%) = temp_point(draw_step).no
   draw_step = draw_step + 1
   Call set_point_coordinate(epolygon(poly_no%).data(0).p.v(i%), epolygon(poly_no%).data(0).p.coord(i%), is_change)
   epolygon(poly_no%).data(0).p.line_no(i% - 1) = set_line(epolygon(poly_no%).data(0).p.v(i% - 1), _
          epolygon(poly_no%).data(0).p.v(i%), 0, 0, pointapi0, pointapi0, pointapi0, _
           depend_condition(point_, epolygon(poly_no%).data(0).p.v(i% - 1)), _
            depend_condition(point_, epolygon(poly_no%).data(0).p.v(i%)), condition, condition_color, 1, 0)
   Call set_parent(polygon_, poly_no%, point_, epolygon(poly_no%).data(0).p.v(i%), 0)
   m_poi(epolygon(poly_no%).data(0).p.v(i%)).data(0).parent.co_degree = 2
  ' m_poi(p.v(i%)).data(0).degree = 0
   'Call get_new_char(p.v(i%))
   'Call set_point_visible(p.v(i%), 1, False)
Next i%
   epolygon(poly_no%).data(0).p.line_no(n%) = set_line(epolygon(poly_no%).data(0).p.v(n% - 1), _
     epolygon(poly_no%).data(0).p.v(0), 0, 0, pointapi0, pointapi0, pointapi0, _
    depend_condition(point_, epolygon(poly_no%).data(0).p.v(n% - 1)), _
     depend_condition(point_, epolygon(poly_no%).data(0).p.v(0)), condition, condition_color, 1, 0)
'    Call from_draw_to_input(0, temp_point(2).no, depend_condition(point_, temp_point(0).no), _
           depend_condition(point_, temp_point(1).no), 0, True, 2)
...
'修改点的名,设置点闪烁
      If draw_new_point(mouse_down_coord, ele1, ele2, blue, False, 1, draw_step) = exist_point Then   '读出已知点
          '输入点的名称
...
ElseIf list_type_for_draw = 1 Then
  ...
  If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 0, draw_step) > 0 Then    '
      move_init = draw_step
...
ElseIf list_type_for_draw = 5 Then '画等长线段
      ...
      ElseIf draw_step = 2 Then
        If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 0, draw_step, True) > 0 Then
          move_init = draw_step
           Move_Enabled = True
           temp_circle(0) = m_circle_number(1, temp_point(2).no, pointapi0, _
                      0, 0, 0, distance_of_two_POINTAPI(m_poi(temp_point(0).no).data(0).data0.coordinate, _
                       ...
                       fill_color, True)
            Call set_parent(point_, temp_point(0).no, circle_, temp_circle(0), _
                              circle_radio_relate_by_two_point_, temp_point(0).no, _
                                                 temp_point(1).no)
            Call set_parent(point_, temp_point(1).no, circle_, temp_circle(0), 0)
           MDIForm1.StatusBar1.Panels(1).text = LoadResString_(1975, "")
       End If
      End If
ElseIf list_type_for_draw = 6 Then '画角平分线
    If draw_step = 0 Then
       If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 0, draw_step) > 0 Then  '画第一点
       move_init = draw_step
       Up_Enabled = True
       Move_Enabled = True
       End If
...
'                 mouse_down_coord = m_poi(temp_point(2).no).data(0).data0.coordinate
    ...
    ElseIf draw_step = 4 Then
          ...
          Call set_forbid_line
          Call set_forbid_circle
        If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 255, draw_step, True) > 0 Then  '
             Call init_draw_data
...
'****************************************************************************
 ...
 ElseIf draw_step = 1 Then
         ...
         Call set_forbid_line
         Call set_forbid_circle
 If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 255, draw_step) <= 0 Then
       draw_step = 0
 Else
...
ElseIf draw_step = 2 Then
 ...
 '选圆
   ...
   ElseIf draw_step = 4 Then
    ...
    If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 5, draw_step) > 0 Then
           Call init_draw_data
            operator = "draw_point_and_line"
            list_type_for_draw = 1
    Else
...
ElseIf draw_step = 6 Then
    If draw_new_point(mouse_down_coord, ele1, ele2, blue, True, 5, draw_step) > 0 Then   '点必需落在切线上
       Call init_draw_data
        operator = "draw_point_and_line"
         list_type_for_draw = 1
    Else
...
ElseIf draw_step = 3 Then '画相切圆
    Call draw_new_point(mouse_down_coord, ele1, ele2, red, True, 0, draw_step)
         'Exit Sub
   '    End If
 ...
 '**********************************************************************
   ElseIf draw_step = 4 Then
     Call draw_new_point(mouse_down_coord, ele1, ele2, red, True, 0, draw_step, True)
       Call init_draw_data
   End If
...
If list_type_for_draw = 1 Then
   ...
   ElseIf draw_step = 2 Then
         ...
         Call set_forbid_line
         Call set_forbid_circle
        If draw_new_point(mouse_down_coord, ele1, ele2, red, True, 255, draw_step, 0) > 0 Then
...
ElseIf draw_step = 2 Then
 ...
 End If
  poly_no% = 0
 'If list_type_for_draw = 1 Then
   Call set_polygon(temp_point(0).no, temp_point(1).no, list_type_for_draw + 2, _
        poly_no%, ty1, True, 0)
 'ElseIf list_type_for_draw = 2 Then
...
If list_type_for_draw = 3 Then
    Call C_curve.set_curve_poi(0, Int(X), Int(Y), 0, 0, 1, yidian_type) '1 画直线
ElseIf list_type_for_draw = 4 Then
    Call C_curve.set_curve_poi(0, Int(X), Int(Y), 0, 0, 2, yidian_type) '2 画圆
...
Select Case list_type_for_draw
 Case 1 '画线或点
       If draw_step = 0 Then
                Call init_draw_data
       ElseIf draw_step = 1 Then
         Call draw_new_point(mouse_up_coord, ele1, ele2, red, True, 255, draw_step)
         Call init_draw_data
       End If
 Case 2, 3, 4
      If draw_step = 1 Then
  ...
  If draw_step = 1 Then
       '画第二个点,
        ...
        m_temp_circle_for_input.data(0).center = temp_point(0).no
            Call draw_temp_circle_for_input
     If draw_new_point(mouse_up_coord, ele1, ele2, red, True, 255, draw_step) > 0 Then
        'temp_circle(0) = m_circle_number(1, 0, m_temp_circle_for_input.data(0).c_coord, temp_point(0).no, _
                        temp_point(1).no, temp_point(2).no, m_temp_circle_for_input.data(0).radii, _
                         0, 0, 0, 1, 1, condition_color, True)
        Call init_draw_data
...
'*************************************************************************************************
   ElseIf list_type_for_draw = 2 Then
    '画出标准线
        If draw_step = 1 Then
            Call draw_temp_line_for_mouse_up(False)
   Dim record0 As total_record_type
         ...
         temp_record.record_data.data0.condition_data.condition(1).ty = wenti_cond_
          temp_record.record_data.data0.condition_data.condition(1).no = _
           C_display_wenti.get_current_wenti_no + 1
         Call set_mid_point(temp_point(0).no, temp_point(2).no, temp_point(1).no, 0, 0, 0, 0, _
              0, record0, 0, 0, 0, 0, 0)
...
End If
 If (list_type_for_draw = 3 Or list_type_for_draw = 4 Or list_type_for_draw = 5) And yidian_type >= 1 Then
 'yidian_type=2画移动点的运动轨道完成。yidian_type=3选择移动点yidian_type=4推动移动点
  If C_curve.set_curve_poi(yidian_no, Int(X), Int(Y), mouse_move_coord.X, mouse_move_coord.Y, 3, yidian_type) Then
      '画移动点的运动轨道完成
   yidian_type = yidian_type + 1 'yidian_type=3画移动点的运动轨道完成
...
Public Function draw_new_point(input_coord As POINTAPI, ele1 As condition_type, _
                                 ele2 As condition_type, draw_red As Boolean, _
                                  is_set_input As Boolean, need_control As Byte, _
                                   ByVal in_draw_step As Integer, _
                                     Optional is_no_need_pre_input As Boolean = False, _
                                      Optional is_need_re_draw As Boolean = True) As Integer
...
Dim point_type%
't_coord = input_coord
     point_type% = read_inter_point(input_coord, ele1, ele2, temp_point(in_draw_step).no, _
                           is_set_input, tangent_line_type, need_control, is_need_re_draw)
 ...
 '                        input_coord = t_coord
      If point_type% > 0 Then
       If point_type% = set_new_wenti_cond Then '输入的点已有,但变动位置,切线
         ...
         draw_new_point = True
       End If
       Call set_point_no_reduce(temp_point(in_draw_step).no, False)
       If draw_red Then
        Call C_display_picture.draw_red_point(temp_point(in_draw_step).no)
       End If
        'draw_new_point = True '读出已知点
               If is_set_input Then
                Call from_draw_to_input(point_type%, temp_point(in_draw_step).no, ele1, ele2, _
                             tangent_line_type, is_no_need_pre_input, in_draw_step)   '新点 引入的输入语句
               End If
      End If
        '选中已知点, 返回点号
End Function
...
For i% = 1 To last_conditions.last_cond(1).line_no
    m_lin(i%).data(0).data0.type = condition
     For j% = 1 To m_lin(i%).data(0).data0.in_point(0) - 1
      If m_lin(i%).data(0).data0.color(j%) <> condition_color And _
           m_lin(i%).data(0).data0.color(j%) <> 0 Then
       m_lin(i%).data(0).data0.color(j%) = condition_color
      m_lin(i%).data(0).is_change = 255
      End If
     Next j%
        Call C_display_picture.re_draw_line(i%)
 Next i%
'For i% = 1 To last_conditions.last_cond(1).circle_no
...
Else
 ...
 End If
End Function
Public Function depend_condition(ByVal ty As Integer, ByVal no%, Optional old_no As Integer) As condition_type
      depend_condition.ty = ty
      depend_condition.no = no%
      depend_condition.old_no = old_no
End Function
...
Public Function set_tangent_line_data(p_coord1 As POINTAPI, p_coord2 As POINTAPI, _
           circ1 As Integer, circ2 As Integer, ele1 As condition_type, ele2 As condition_type, _
                          visible As Byte, tangent_line_ty As Integer) As Integer
Dim i%
For i% = last_conditions.last_cond(1).tangent_line_no + 1 To last_conditions.last_cond(0).tangent_line_no  '已有数据且未进入数据库的切线
...
End If
   ...
   set_tangent_line_data = last_conditions.last_cond(0).tangent_line_no
...
set_tangent_circle_data_mark1:
  ...
  If tangent_circle_no = 0 Then
    ...
    Call draw_red_tangent_point(tangent_point_coord_1, set_tangent_circle_data)
    Call draw_red_tangent_point(tangent_point_coord_2, set_tangent_circle_data)
  Else
    Call draw_tangent_circle(set_tangent_circle_data)
    Call draw_red_tangent_point(tangent_point_coord_1, set_tangent_circle_data)
    Call draw_red_tangent_point(tangent_point_coord_2, set_tangent_circle_data)
...
'****************************************************************************************************************
         ...
         Call set_forbid_circle
'****************************************************************************************************************
       If draw_new_point(mouse_up_coord, ele1, ele2, red, True, 255, draw_step) > 0 Then  '输入坐标点
...
End Function
Public Sub draw_aid_line_for_display(ByVal line_no%)
 If line_no% > 0 And m_lin(line_no%).data(0).data0.visible = 0 Then
  m_lin(line_no%).data(0).data0.visible = 1
  m_lin(line_no%).data(0).data0.total_color = conclusion_color
  m_lin(line_no%).data(0).data0.color(1) = conclusion_color
  m_lin(line_no%).data(0).is_change = 255
  Draw_form.DrawStyle = 1
  Draw_form.DrawWidth = 1
...
End Sub
Public Sub draw_aid_circle_for_display(ByVal circle_no%)
 If circle_no% > 0 And m_Circ(circle_no%).data(0).data0.visible = 0 Then
  m_Circ(circle_no%).data(0).data0.visible = 1
  m_Circ(circle_no%).data(0).data0.color = conclusion_color
  m_lin(circle_no%).data(0).is_change = 255
  Draw_form.DrawStyle = 1
  Draw_form.DrawWidth = 1
...
Public Sub draw_aid_picture(ByVal data_type As Integer, ByVal data_no%)
...
'Case new_point_
Case V_line_value_ '
    Call draw_aid_line_for_display(V_line_value(data_no%).data(0).v_line)
...
Case sides_length_of_triangle_
 Call draw_aid_triangle(Sides_length_of_triangle(data_no%).data(0).triangle)
'stri(0) = Sides_length_of_triangle(n%).data(0).record.display_string
        Call draw_aid_triangle(Sides_length_of_triangle(data_no%).data(0).triangle)
Case area_of_circle_
 Call draw_aid_circle_for_display(area_of_circle(data_no%).data(0).circ)
...
Case sides_length_of_circle_
 Call draw_aid_circle_for_display(Sides_length_of_circle(data_no%).data(0).circ)
...
Case two_line_value_
 Call draw_aid_line_for_display(two_line_value(data_no%).data(0).data0.line_no(0))
 Call draw_aid_line_for_display(two_line_value(data_no%).data(0).data0.line_no(1))
Case tixing_
...
Case tangent_line_
  Call draw_aid_line_for_display(tangent_line(data_no%).data(0).line_no)
Case equal_side_triangle_
 Call draw_aid_triangle(equal_side_triangle(data_no%).data(0).triangle)
Case equal_side_right_triangle_
 Call draw_aid_triangle(equal_side_right_triangle(data_no%).data(0).triangle)
Case epolygon_
...
'**********************************************************
Case angle3_value_
 Call draw_aid_angle(angle3_value(data_no%).data(0).data0.angle(0))
 Call draw_aid_angle(angle3_value(data_no%).data(0).data0.angle(1))
 Call draw_aid_angle(angle3_value(data_no%).data(0).data0.angle(2))
'**********************************************************
Case Dangle_
 Call draw_aid_angle(Dangle(data_no%).data(0).angle)
'************************************************************
Case Dline_
...
'End If
 ...
 ' Call draw_aid_line_for_display( dpoint_pair(n%).
'*******************************************************
Case eline_
'If concl_or_cond = 0 Then
...
're1 = Deline(n%).data(0).record
  Call draw_aid_line_for_display(Deline(data_no%).data(0).data0.line_no(0))
  Call draw_aid_line_for_display(Deline(data_no%).data(0).data0.line_no(1))
Case midpoint_ 'equal_segment_on_line
' stri(0) = set_display_mid_point(Dmid_point(n%), _
'    0, True, is_depend)
 're1 = Dmid_point(n%).data(0).record
  Call draw_aid_line_for_display(Dmid_point(data_no%).data(0).data0.line_no)
'***********************************************************
...
'    Next i%
' End If
' Call draw_aid_line_for_display(mid_point_line(data_no%).data(0).poi())
...
'     End If
   Call draw_aid_line_for_display(line_value(data_no%).data(0).data0.line_no)
'**********************************************************
Case line3_value_
'   stri(0) = set_display_three_line_value(line3_value(n%).data(0), True, dis_ty, is_depend)
'' re1 = line3_value(n%).data(0).record
    Call draw_aid_line_for_display(line3_value(data_no%).data(0).data0.line_no(0))
    Call draw_aid_line_for_display(line3_value(data_no%).data(0).data0.line_no(1))
    Call draw_aid_line_for_display(line3_value(data_no%).data(0).data0.line_no(2))
    Call draw_aid_line_for_display(line3_value(data_no%).data(0).data0.line_no(3))
    Call draw_aid_line_for_display(line3_value(data_no%).data(0).data0.line_no(4))

'*****************************************************
Case paral_
 Call draw_aid_line_for_display(Dparal(data_no%).data(0).data0.line_no(0))
 Call draw_aid_line_for_display(Dparal(data_no%).data(0).data0.line_no(1))
'*********************************************
Case parallelogram_
...
Case point3_on_line_
    'Call draw_aid_line_for_display(three_point_on_line(data_no%).data(0).is_no_initial)
'call draw_aid_line_for_display(three_point_on_line(data_no%).data(0).
'****************************************************************
Case point4_on_circle_
 Call draw_aid_circle_for_display(four_point_on_circle(data_no%).data(0).circ)
'For i% = 0 To 3
...
'End If
Case relation_
 Call draw_aid_line_for_display(Drelation(data_no%).data(0).data0.line_no(0))
 Call draw_aid_line_for_display(Drelation(data_no%).data(0).data0.line_no(1))
...
Case similar_triangle_
 Call draw_aid_triangle(Dsimilar_triangle(data_no%).data(0).triangle(0))
 Call draw_aid_triangle(Dsimilar_triangle(data_no%).data(0).triangle(1))
'****************************************************
Case tri_function_
'*************************************************************
Case total_equal_triangle_
 Call draw_aid_triangle(Dtotal_equal_triangle(data_no%).data(0).triangle(0))
 Call draw_aid_triangle(Dtotal_equal_triangle(data_no%).data(0).triangle(1))
Case verti_
 Call draw_aid_line_for_display(Dverti(data_no%).data(0).line_no(0))
 Call draw_aid_line_for_display(Dverti(data_no%).data(0).line_no(1))
'*********************************************************
'*************************************************
Case verti_mid_line_
 Call draw_aid_line_for_display(verti_mid_line(data_no%).data(0).data0.line_no(0))
 Call draw_aid_line_for_display(verti_mid_line(data_no%).data(0).data0.line_no(0))
'*********************************************************
End Select
End Sub

Public Sub draw_aid_triangle(ByVal triangle_no%)
Call draw_aid_line_for_display(triangle(triangle_no%).data(0).line_no(0))
Call draw_aid_line_for_display(triangle(triangle_no%).data(0).line_no(1))
Call draw_aid_line_for_display(triangle(triangle_no%).data(0).line_no(2))
End Sub

Public Sub draw_aid_angle(ByVal angle_no%)
Call draw_aid_line_for_display(angle(angle_no%).data(0).line_no(0))
Call draw_aid_line_for_display(angle(angle_no%).data(0).line_no(1))
End Sub
Public Sub draw_red_tangent_point(in_coord As POINTAPI, tangent_circle_no As Integer)
Dim temp_red_tangent_point As Integer
If in_coord.X > 0 And in_coord.Y > 0 Then
 temp_red_tangent_point = read_point(in_coord, 0)
 If red_tangent_point = 0 And temp_red_tangent_point > 0 And _
      tangent_circle_with_red_tangent_point = 0 Then
    red_tangent_point = temp_red_tangent_point
     tangent_circle_with_red_tangent_point = tangent_circle_no
    'm_poi(red_tangent_point).data(0).data0.color = conclusion_color
    'm_poi(red_tangent_point).data(0).is_change = True
    Call C_display_picture.draw_red_point(red_tangent_point)
 ElseIf red_tangent_point > 0 And temp_red_tangent_point = 0 And _
      tangent_circle_with_red_tangent_point = tangent_circle_no Then
    'm_poi(red_tangent_point).data(0).data0.color = condition_color
    'm_poi(red_tangent_point).data(0).is_change = True
    Call C_display_picture.redraw_point(red_tangent_point)
         red_tangent_point = 0
          tangent_circle_with_red_tangent_point = 0
 End If
 End If
End Sub
Public Function compare_two_point_on_circle(p1%, p0%, p2%) As Boolean
Dim t_l(1) As POINTAPI
t_l(0) = minus_POINTAPI(m_poi(p1%).data(0).data0.coordinate, _
               m_poi(p0%).data(0).data0.coordinate)
t_l(1) = minus_POINTAPI(m_poi(p2%).data(0).data0.coordinate, _
               m_poi(p0%).data(0).data0.coordinate)
If t_l(0).X * t_l(1).Y - t_l(0).Y * t_l(1).X > 0 Then
   compare_two_point_on_circle = True
Else
   compare_two_point_on_circle = False
End If
End Function
