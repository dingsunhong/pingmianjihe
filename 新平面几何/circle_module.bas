...
Public Function set_circle_from_four_point(ByVal p1%, ByVal p2%, ByVal p3%, ByVal p4%, re As total_record_type) As Integer
  set_circle_from_four_point = m_circle_number(1, 0, pointapi0, p1%, p2%, p3%, 0, 0, 0, 0, 0, 0, 0, False)
   If set_circle_from_four_point = 0 Then
      set_circle_from_four_point = m_circle_number(1, 0, pointapi0, p2%, p3%, p4%, 0, 0, 0, 0, 0, 0, 0, False)
       If set_circle_from_four_point = 0 Then
         set_circle_from_four_point = m_circle_number(1, 0, pointapi0, p3%, p4%, p1%, 0, 0, 0, 0, 0, 0, 0, False)
         If set_circle_from_four_point% = 0 Then
            set_circle_from_four_point = m_circle_number(1, 0, pointapi0, p4%, p1%, p2%, 0, 0, 0, 0, 0, 0, 0, False)
             If set_circle_from_four_point = 0 Then
              set_circle_from_four_point = m_circle_number(1, 0, pointapi0, p1%, p2%, p3%, 0, 0, 0, 0, 0, _
                                  condition, condition_color, True)
               Call add_point_to_m_circle(p4%, set_circle_from_four_point, re, True)
             Else
               Call add_point_to_m_circle(p4%, set_circle_from_four_point, re, True)
             End If
         Else
           Call add_point_to_m_circle(p2%, set_circle_from_four_point, re, True)
         End If
       Else
        Call add_point_to_m_circle(p1%, set_circle_from_four_point, re, True)
       End If
   Else
    Call add_point_to_m_circle(p4%, set_circle_from_four_point, re, True)
   End If
...
set_m_circle_data_mark1:
 ...
 End If
 If c_data0.data0.center = 0 Then
    c_data0.data0.my_name(0) = "⊙[down({{0}}" + _
                 m_poi(c_data0.data0.in_point(1)).data(0).data0.name + _
                 m_poi(c_data0.data0.in_point(2)).data(0).data0.name + _
                 m_poi(c_data0.data0.in_point(3)).data(0).data0.name + ")]"
    c_data0.data0.my_name(1) = "⊙(" + _
                 m_poi(c_data0.data0.in_point(1)).data(0).data0.name + _
                 m_poi(c_data0.data0.in_point(2)).data(0).data0.name + _
                 m_poi(c_data0.data0.in_point(3)).data(0).data0.name + ")"
                 
 ElseIf c_data0.data0.in_point(1) > 0 And c_data0.data0.center > 0 Then
    c_data0.data0.my_name(0) = "⊙{{0}}" + m_poi(c_data0.data0.center).data(0).data0.name + "[down({{1}}" + _
                 m_poi(c_data0.data0.in_point(1)).data(0).data0.name + ")]"
     c_data0.data0.my_name(1) = "⊙" + m_poi(c_data0.data0.center).data(0).data0.name + "(" + _
                 m_poi(c_data0.data0.in_point(1)).data(0).data0.name + ")"
...
End If
End Function
Public Function add_point_to_m_circle(n_point%, circle_no%, record As total_record_type, is_set_data As Byte, _
            Optional is_input As Boolean = True) As Byte
    'is_set_data =0 只加点，=1 只记录数据 =255加点并记录 is_input = true 才能调用sei_parent
Dim i%, j%, k%, tl%
Dim ele_(1) As condition_type
Dim temp_record As total_record_type
If n_point% = 0 Or circle_no% = 0 Then
 Exit Function
ElseIf is_set_data = 1 Then '只记录数据，已有点在圆上
 GoTo set_data_mark
End If
If n_point% <> 0 Then
...
'******************************************************************************************************************
 ...
 Else
   ...
   End If
 End If
 If is_set_data > 0 Then '输入新数据
set_data_mark:
Call add_record_to_record(record.record_data.data0.condition_data, temp_record.record_data.data0.condition_data)
  'add_point_to_m_circle = add_point_to_circle_(n_point%, circle_no%)
   If m_Circ(circle_no%).data(0).data0.in_point(0) >= 2 Then
      For i% = 1 To m_Circ(circle_no%).data(0).data0.in_point(0) - 1
       If m_Circ(circle_no%).data(0).data0.center > 0 Then
       Call set_three_angle_value(Abs(angle_number(n_point%, m_Circ(circle_no%).data(0).data0.in_point(i%), _
                  m_Circ(circle_no%).data(0).data0.center, "", 0)), _
                   Abs(angle_number(m_Circ(circle_no%).data(0).data0.in_point(i%), n_point%, _
                  m_Circ(circle_no%).data(0).data0.center, "", 0)), 0, "1", "-1", "0", "0", 0, temp_record, 0, 0, 0, 254, 0, 0, False)
       End If
       For j% = i% + 1 To m_Circ(circle_no%).data(0).data0.in_point(0) - 1
        Call set_three_point_on_circle(m_Circ(circle_no%).data(0).data0.in_point(i%), _
          m_Circ(circle_no%).data(0).data0.in_point(j%), n_point%, _
            m_Circ(circle_no%).data(0).data0.center, circle_no%, temp_record)
        For k% = j% + 1 To m_Circ(circle_no%).data(0).data0.in_point(0) - 1
         Call set_four_point_on_circle(m_Circ(circle_no%).data(0).data0.in_point(i%), _
               m_Circ(circle_no%).data(0).data0.in_point(j%), m_Circ(circle_no%).data(0).data0.in_point(k%), _
                 n_point%, circle_no%, temp_record, 0, 254)
        Next k%
       Next j%
      Next i%
   End If
                  ...
                  Call add_tangent_line_to_circle0( _
                    m_Circ(circle_no%).data(0).data0.tangent_line.tangent_celement(i).tangent_element_no, _
                     m_Circ(circle_no%).data(0).data0.tangent_line.tangent_celement(i).tangent_point, _
                      m_Circ(circle_no%).data(0).data0.in_point(j%), n_point%, temp_record)
           End If
       Next j%
     Next i%
  End If
...
combine_two_circle_mark2:
 Call add_point_to_m_circle(m_Circ(c2%).data(0).data0.in_point(i%), c1%, record0, 255)
        combine_two_circle = True
Next i%
Exit Function
